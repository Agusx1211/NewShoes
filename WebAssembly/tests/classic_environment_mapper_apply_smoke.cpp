// Smoke: exercise the *original* WW3D2 ClassicEnvironmentMapperClass::Apply
// path (GeneralsMD/Code/Libraries/Source/WWVegas/WW3D2/mapper.cpp) under the
...
// ClassicEnvironmentMapperClass::Apply emits so a future browser
// camera-space-normal environment-map bridge can diff against the real
// emission rather than a hand-written approximation. We do NOT stub or
// reimplement Apply or Calculate_Texture_Matrix; we construct the real
// ClassicEnvironmentMapperClass on a nonzero stage and call the real Apply().
//
...
int main()
{
	...
	// Construct the real ClassicEnvironmentMapperClass on a NONZERO stage
	// (stage 1), as the original environment-mapped mesh material users do.
	// NEW_REF goes through the original RefCountClass allocator; we release it
	// at the end.
	constexpr int kStage = 1;
	ClassicEnvironmentMapperClass *mapper = NEW_REF(ClassicEnvironmentMapperClass, (kStage));
	check(mapper != nullptr, "ClassicEnvironmentMapperClass allocation failed");
	if (mapper != nullptr) {
		check(mapper->Mapper_ID() == TextureMapperClass::MAPPER_ID_CLASSIC_ENVIRONMENT,
			"Mapper_ID must be MAPPER_ID_CLASSIC_ENVIRONMENT");
		check(mapper->Needs_Normals() == true,
			"ClassicEnvironmentMapperClass must request normals");
	...
	// ---------------------------------------------------------------------------
	// 2. stage TEXCOORDINDEX == D3DTSS_TCI_CAMERASPACENORMAL
	// ---------------------------------------------------------------------------
	DWORD stage_texcoord_index = 0;
	if (device != nullptr) {
		device->GetTextureStageState(kStage, D3DTSS_TEXCOORDINDEX, &stage_texcoord_index);
	}
	check(stage_texcoord_index == D3DTSS_TCI_CAMERASPACENORMAL,
		"stage TEXCOORDINDEX must be CAMERASPACENORMAL");
	...
	if (g_failures) {
		std::fprintf(stderr, "classic-environment-mapper-apply-smoke: failures reported above\n");
		return 1;
	}

	std::printf(
		"{\"ok\":true,\"smoke\":\"classic-environment-mapper-apply\","
		"\"source\":\"GeneralsMD/Code/Libraries/Source/WWVegas/WW3D2/mapper.cpp ClassicEnvironmentMapperClass::Apply\","
		"\"stage\":%d,"
		"\"transformState\":%lu,"
		...
		static_cast<unsigned long>(static_cast<DWORD>(D3DTSS_TCI_CAMERASPACENORMAL)),
