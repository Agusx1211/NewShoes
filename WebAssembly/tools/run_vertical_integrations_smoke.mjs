...
const steps = [
  {
    ...
    validate(payload) {
      ...
      expect(payload.covered?.includes("runtime original GameLogic::processCommandList dispatch of MSG_NEW_GAME through prepareNewGame, original ScriptEngine::setGlobalDifficulty, original Shell::hideShell, and first-call startNewGame(FALSE) deferral"),
        "startup vertical smoke did not cover runtime original GameLogic MSG_NEW_GAME dispatch through original ScriptEngine/Shell", payload);
...
console.log(JSON.stringify({
  ...
  covered: [
    "aggregate startup vertical covering browser GameEngine.cpp startup frontier, original GameEngine lifetime, original Miles openDevice, W3D window/layout ownership, source-pinned GameLogic MSG_NEW_GAME dispatch frontier, and runtime original GameLogic::processCommandList first-call deferral through original ScriptEngine::setGlobalDifficulty and Shell::hideShell",
  ...
  nextRequired: [
    ...
    "replace focused runtime PlayerList/GlobalData adapters before continuing deferred startNewGame into terrain/player/script load",
