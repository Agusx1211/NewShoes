...
const browserChecks = [
  {
    ...
    validate(payload) {
      ...
      expect(payload.archiveBackedStartup.functionLexiconRuntime?.status === 'base_function_lexicon_motd_runtime_owned',
        'Archive-backed startup boot did not report the expected MOTD FunctionLexicon frontier');
      expect(payload.archiveBackedStartup.functionLexiconRuntime?.lookups?.popupReplayInput === true
        ...
        'Archive-backed startup boot did not prove PopupReplay modal, ControlBar input, MOTD, OptionsMenu, and SkirmishMapSelectMenu callback lookups');
