  ...
  },
  {
    ...
    validate(payload) {
      ...
      expect(payload.runtimeTargetBoundary?.originalGameLogicCppLinked === true
        ...
        'GameLogic new-game dispatch frontier did not prove the focused original runtime target with GlobalData/ScriptEngine/Shell ownership');
...
const steps = [
  ...
  },
  {
    ...
    validate(payload) {
      ...
      expect(payload.source === 'GeneralsMD original GlobalData.cpp/GameLogic.cpp/GameLogicDispatch.cpp/ScriptEngine.cpp',
        'GameLogic new-game runtime smoke did not link the original GlobalData/GameLogic sources');
      ...
      expect(payload.originalOwners?.includes('GlobalData TheWritableGlobalData')
        ...
        'GameLogic new-game runtime smoke did not report original GlobalData/ScriptEngine/Shell ownership');
    },
  },
];
...
console.log(JSON.stringify({
  ...
  covered: [
    ...
    'runtime original GameLogic::processCommandList dispatch of MSG_NEW_GAME through prepareNewGame, original GlobalData TheWritableGlobalData, original ScriptEngine::setGlobalDifficulty, original Shell::hideShell, and first-call startNewGame(FALSE) deferral',
  ],
  nextRequired: [
    ...
    'replace the focused runtime PlayerList and in-memory BlankWindow adapters before continuing deferred startNewGame into terrain/player/script load',
  ],
