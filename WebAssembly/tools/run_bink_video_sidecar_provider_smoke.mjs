...
if (process.argv.length < 4) {
  console.error(
    "usage: node tools/run_bink_video_sidecar_provider_smoke.mjs " +
      "artifacts/real-assets/GC_Background.bik artifacts/real-assets/VS_small.bik",
  );
  console.error("Run npm run transcode:bink-video first if the files are missing.");
  process.exit(1);
}
...
for (const path of [gcPath, vsPath, manifestPath]) {
  if (!existsSync(path)) {
    console.error(`Bink sidecar provider input not found: ${path}`);
    console.error("Run npm run transcode:bink-video first.");
    process.exit(1);
  }
}
