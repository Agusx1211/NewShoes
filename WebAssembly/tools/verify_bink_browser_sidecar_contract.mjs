...
// runtime. It is intentionally forward-looking: the open M8 TODO is to wire
// the browser Bink runtime to this manifest, so this verifier pins the
// contract that the next provider change must satisfy without modifying the
// C++ provider itself.
//
// Pinned contract:
...
//      schema "cnc-zh-bink-browser-video-manifest/v1" (the schema/path the
//      provider runtime must know).
...
function verifyProviderInvariant(provider, errors, facts) {
  ...
  // Forward-looking probe (fact, not an error): does the provider currently
  // reference the generated manifest schema/path? This documents the open M8
  // wiring TODO without failing the contract, since wiring is the next step.
  ...
  facts.providerReferencesManifestPath = manifestNameRef.test(provider.text);
}
...
async function main() {
  ...
  const report = {
    ...
    note:
      ...
      "and the browser Bink provider. The provider is not modified here; this " +
      "verifier documents the manifest schema/path, the source -> sidecar " +
      "association, the original-style path aliases the runtime must resolve, " +
      "and the decode-readiness invariant that ties WasmBinkProviderCanDecodeFrames " +
      "to real BinkCopyToBuffer pixel copies.",
  };
