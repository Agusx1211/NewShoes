...
| `WWVegas/WW3D2` | Partial | `zh_ww3d2_compile_frontier` now builds 82 original WW3D2 sources: hierarchy/animation/LOD and render-object definitions (`animobj.cpp`, `agg_def.cpp`, `hanim.cpp`, `hanimmgr.cpp`, `hcanim.cpp`, `hlod.cpp`, `hmorphanim.cpp`, `hmdldef.cpp`, `hrawanim.cpp`, `htree.cpp`, `htreemgr.cpp`, `distlod.cpp`, `composite.cpp`, `nullrobj.cpp`, `rendobj.cpp`, `soundrobj.cpp`, `renderobjectrecycler.cpp`), spatial/collision/visibility and scene helpers (`aabtree.cpp`, `aabtreebuilder.cpp`, `camera.cpp`, `coltest.cpp`, `intersec.cpp`, `meshgeometry.cpp`, `proto.cpp`, `scene.cpp`, `visrasterizer.cpp`, `bw_render.cpp`, `bwrender.cpp`), light/projector/decal/particle/snap-point support (`light.cpp`, `projector.cpp`, `decalsys.cpp`, `decalmsh.cpp`, `part_emt.cpp`, `part_ldr.cpp`, `snappts.cpp`, `shattersystem.cpp`), asset/status/cache/exclusion and W3D dependency helpers (`assetstatus.cpp`, `bitmaphandler.cpp`, `texfcach.cpp`, `texturefile.cpp`, `texturethumbnail.cpp`, `w3d_dep.cpp`, `w3d_util.cpp`, `w3dexclusionlist.cpp`), mesh/material/metadata and algorithm helpers (`matinfo.cpp`, `matpass.cpp`, `matrixmapper.cpp`, `mesh.cpp`, `meshbuild.cpp`, `meshdam.cpp`, `metalmap.cpp`, `motchan.cpp`, `polyinfo.cpp`, `predlod.cpp`, `prim_anim.cpp`, `static_sort_list.cpp`, `stripoptimizer.cpp`), Direct3D-adjacent format/render helpers (`bmp2d.cpp`, `boxrobj.cpp`, `dx8fvf.cpp`, `dx8polygonrenderer.cpp`, `dx8rendererdebugger.cpp`, `dx8texman.cpp`, `formconv.cpp`, `line3d.cpp`, `linegrp.cpp`, `render2d.cpp`, `ringobj.cpp`, `segline.cpp`, `seglinerenderer.cpp`, `streak.cpp`, `streakRender.cpp`, `txt2d.cpp`, `ww3dformat.cpp`), and small support/debug/text files (`font3d.cpp`, `layer.cpp`, `pivot.cpp`, `pot.cpp`, `rinfo.cpp`, `statistics.cpp`, `txt.cpp`). This is compile coverage only. The current Direct3D 8 shim is a declaration/type surface for compiling original code, not a renderer implementation; the runtime backend (`dx8wrapper.cpp`, caps, vertex/index buffers, renderer, texture/material upload paths), DDS/DirectDraw helpers, GDI text rendering, Video-for-Windows frame grabbing, Miles/WWAudio binding, Surrender `sr*` utility headers, and render-object/light runtime linking still need browser ports before WW3D2 can be runtime-complete. |
...
| `DX90SDK` | Partial | Target-local `D3DX8Math.h` shim covers vector4/matrix operations used by original `GameEngine/Common` Bezier helpers and the `D3DXMatrixInverse` path used by original `WWMath/matrix3d.cpp`; a lowercase `D3dx8math.h` wrapper handles the original include spelling on case-sensitive filesystems. A declaration-only `d3d8.h` / D3DX core surface now covers the Direct3D formats, FVF helpers, COM interface declarations, and constants needed by the current WW3D2 compile frontier. Broader DirectX 8 runtime compatibility for WW3D and W3D device code remains open and must map to WebGL2/WebGPU rather than native DirectX. |
...
`D3dx8core.h`, `D3DXMath.h`, `Dx8Wrapper.h`, `font.h`,
...
- `zh_ww3d2_compile_frontier`: 82 original `WWVegas/WW3D2`
  ...
  texture-file metadata, material metadata, primitive animation, text-texture,
  D3D format/FVF, early 2D/render helper, render-info, debug, and utility
  leaves compiled into a wasm static library as the current renderer-library
  compile frontier.
