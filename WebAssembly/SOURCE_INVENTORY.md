...
| `WWVegas/wwshade` | Partial | `zh_wwshade_compile_frontier` now builds 17 original shader/material sources that do not require generated shader blob headers: `shdbumpdiff.cpp`, `shdbumpspec.cpp`, `shdcubemap.cpp`, `shddef.cpp`, `shddeffactory.cpp`, `shddefmanager.cpp`, `shdforcelinks.cpp`, `shdglossmask.cpp`, `shdhwshader.cpp`, `shdinterface.cpp`, `shdlegacyw3d.cpp`, `shdlib.cpp`, `shdloader.cpp`, `shdmesh.cpp`, `shdrenderer.cpp`, `shdsimple.cpp`, and `shdsubmesh.cpp`. This is compile coverage only until Direct3D shader assembly, shader creation, and material rendering are mapped onto the browser renderer. The six DX6/DX7/DX8 bump variants remain out of the frontier because their generated `*.vsh_code.h` / `*.psh_code.h` shader blob headers are not present in the tree. |
...
| `DX90SDK` | Partial | Target-local `D3DX8Math.h` shim covers vector3/vector4/matrix operations used by original `GameEngine/Common` Bezier helpers, original `WWMath/matrix3d.cpp`, and the current WW3D2/wwshade sorting, point-group, light, and shader paths; lowercase/case-variant wrappers handle original `D3dx8math.h`, `d3dx8math.h`, `d3dx8.h`, `D3dx8core.h`, and `D3dx8tex.h` include spellings on case-sensitive filesystems. Declaration-only `d3d8.h`, `d3d8types.h`, `d3d8caps.h`, and `ddraw.h` surfaces now cover Direct3D formats, FVF helpers, COM interface declarations, DirectDraw DDS caps, D3DX texture creation/load/filter declarations, D3DX shader-buffer/assembly declarations, and constants needed by the current WW3D2 and wwshade compile frontiers. Broader DirectX 8 runtime compatibility for WW3D and W3D device code remains open and must map to WebGL2/WebGPU rather than native DirectX. |
...
- `zh_wwshade_compile_frontier`: 17 original `WWVegas/wwshade`
  shader/material definition, factory, manager, interface, loader,
  mesh/submesh, renderer, simple/gloss/cubemap/legacy W3D, and hardware-shader
  helper sources compiled into a wasm static library. The DX6/DX7/DX8 bump
  variants remain out until their generated shader blob headers are restored or
  regenerated.
