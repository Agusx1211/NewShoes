...
| `GameEngine/Common` | Partial | Original-source core slice now compiles to wasm across memory, strings, file/BIG archive, compression/data-chunk, language, type masks, geometry, terrain, multiplayer settings, timing, RTS accounting and metadata (`Energy.cpp`, `Handicap.cpp`, `MissionStats.cpp`, `Money.cpp`, `PlayerTemplate.cpp`, `ProductionPrerequisite.cpp`, `Science.cpp`, `SpecialPower.cpp`), message streams, audio request/music/event/manager metadata (`AudioEventRTS.cpp`, `GameAudio.cpp`, `GameMusic.cpp`, `GameSounds.cpp`, dynamic audio), and the first INI leaf/parser wrapper sources (`INIAudioEventInfo.cpp`, `INICommandSet.cpp`, `INIControlBarScheme.cpp`, `INIDamageFX.cpp`, `INIMapData.cpp`, `INIModel.cpp`, `INIMiscAudio.cpp`, `INIMultiplayer.cpp`, `INISpecialPower.cpp`). Node smoke coverage exercises the linked non-rendering Common behavior, real BIG reads, and terrain/type/string/file paths. The audio/INI/RTS/message additions are compile coverage only until the real `Common/INI.cpp` reader, full audio manager, Xfer, GlobalData, Thing, Player, ControlBar/DamageFX managers, and GameLogic surfaces replace the current target-local shims; `parseScience`, `parseScienceVector`, and translated-label parsing are temporary bridge helpers. |
...
- `zh_gamenetwork_core`: original `GameEngine/GameNetwork` command/frame and
  ...
  `NetMessageStream.cpp`, `NetPacket.cpp`, `NetworkUtil.cpp`, and `User.cpp`
  compiled into a wasm static library.
