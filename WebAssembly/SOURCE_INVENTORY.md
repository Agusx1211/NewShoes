...
- `gamenetwork-core-smoke`: a Node-executed wasm smoke test that verifies
  ...
  plus control/disconnect/router/wrapper/file-announce/frame-resend packet
  round-trips, `FileTransfer.cpp` map-path helpers, `FrameMetrics.cpp`
  init/reset/cushion behavior, and pooled `User` value semantics.
  `Connection.cpp`, the `FileTransfer.cpp` transfer loop, `FrameMetrics.cpp`
