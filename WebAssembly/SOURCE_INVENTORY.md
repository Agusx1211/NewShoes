...
- `gamenetwork-core-smoke`: a Node-executed wasm smoke test that verifies
  ...
  sorting/deduplication, `NetPacket` frame/run-ahead/chat/progress/file-progress
  packet round-trips, and pooled `User` value semantics. `Connection.cpp`,
  `FileTransfer.cpp`, and `NetMessageStream.cpp` are currently compile coverage
  only.
