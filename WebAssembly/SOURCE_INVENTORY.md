...
ChallengeLoadScreen runtime playback, full campaign-owned load-screen setup,
campaign `finishSinglePlayerInit` ownership, and the broader
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  blank-window first-window `setVideoBuffer`, and focused original
  `SinglePlayerLoadScreen::init` ownership, while native `.bik` frame decoding,
  audio sync, full WebCodecs/`<video>` runtime ownership, full
  ChallengeLoadScreen playback, and campaign-owned load-screen / score-screen
  playback remain open. The runtime archive extractor
