...
| `WWVegas/WW3D2` | Partial | `zh_ww3d2_compile_frontier` now builds 105 original WW3D2 sources: hierarchy/animation/LOD and render-object definitions (`animobj.cpp`, `agg_def.cpp`, `hanim.cpp`, `hanimmgr.cpp`, `hcanim.cpp`, `hlod.cpp`, `hmorphanim.cpp`, `hmdldef.cpp`, `hrawanim.cpp`, `htree.cpp`, `htreemgr.cpp`, `distlod.cpp`, `composite.cpp`, `nullrobj.cpp`, `rendobj.cpp`, `soundrobj.cpp`, `renderobjectrecycler.cpp`), spatial/collision/visibility and scene helpers (`aabtree.cpp`, `aabtreebuilder.cpp`, `camera.cpp`, `coltest.cpp`, `intersec.cpp`, `meshgeometry.cpp`, `proto.cpp`, `scene.cpp`, `visrasterizer.cpp`, `bw_render.cpp`, `bwrender.cpp`), light/projector/decal/particle/snap-point support (`light.cpp`, `lightenvironment.cpp`, `projector.cpp`, `decalsys.cpp`, `decalmsh.cpp`, `part_buf.cpp`, `part_emt.cpp`, `part_ldr.cpp`, `snappts.cpp`, `shattersystem.cpp`, `sphereobj.cpp`), asset/status/cache/exclusion and W3D dependency helpers (`assetstatus.cpp`, `assetmgr.cpp`, `bitmaphandler.cpp`, `texfcach.cpp`, `texturefile.cpp`, `texturethumbnail.cpp`, `w3d_dep.cpp`, `w3d_util.cpp`, `w3dexclusionlist.cpp`), mesh/material/metadata and algorithm helpers (`matinfo.cpp`, `matpass.cpp`, `matrixmapper.cpp`, `mesh.cpp`, `meshbuild.cpp`, `meshdam.cpp`, `meshmatdesc.cpp`, `meshmdl.cpp`, `meshmdlio.cpp`, `metalmap.cpp`, `motchan.cpp`, `polyinfo.cpp`, `predlod.cpp`, `prim_anim.cpp`, `static_sort_list.cpp`, `stripoptimizer.cpp`, `dynamesh.cpp`), Direct3D-adjacent format/render/texture helpers (`bmp2d.cpp`, `boxrobj.cpp`, `ddsfile.cpp`, `dx8caps.cpp`, `dx8fvf.cpp`, `dx8indexbuffer.cpp`, `dx8polygonrenderer.cpp`, `dx8renderer.cpp`, `dx8rendererdebugger.cpp`, `dx8texman.cpp`, `dx8vertexbuffer.cpp`, `formconv.cpp`, `line3d.cpp`, `linegrp.cpp`, `missingtexture.cpp`, `render2d.cpp`, `ringobj.cpp`, `segline.cpp`, `seglinerenderer.cpp`, `shader.cpp`, `sortingrenderer.cpp`, `streak.cpp`, `streakRender.cpp`, `surfaceclass.cpp`, `texture.cpp`, `texturefilter.cpp`, `textureloader.cpp`, `vertmaterial.cpp`, `txt2d.cpp`, `ww3dformat.cpp`), and small support/debug/text files (`dazzle.cpp`, `font3d.cpp`, `layer.cpp`, `mapper.cpp`, `pivot.cpp`, `pot.cpp`, `rinfo.cpp`, `statistics.cpp`, `txt.cpp`). This is compile coverage only. The current Direct3D 8 shim is a declaration/type surface for compiling original code, not a renderer implementation; the full runtime backend (`dx8wrapper.cpp`, `ww3d.cpp`), GDI text rendering, Video-for-Windows frame grabbing, Miles/WWAudio binding, Surrender `sr*` utility headers, WWLib MPU timing, text include compatibility, and render-object/light runtime linking still need browser ports before WW3D2 can be runtime-complete. |
...
| `DX90SDK` | Partial | Target-local `D3DX8Math.h` shim covers vector3/vector4/matrix operations used by original `GameEngine/Common` Bezier helpers, original `WWMath/matrix3d.cpp`, and the current WW3D2 sorting path; lowercase/case-variant wrappers handle original `D3dx8math.h`, `d3dx8.h`, `D3dx8core.h`, and `D3dx8tex.h` include spellings on case-sensitive filesystems. Declaration-only `d3d8.h`, `d3d8types.h`, `d3d8caps.h`, and `ddraw.h` surfaces now cover Direct3D formats, FVF helpers, COM interface declarations, DirectDraw DDS caps, and constants needed by the current WW3D2 compile frontier. Broader DirectX 8 runtime compatibility for WW3D and W3D device code remains open and must map to WebGL2/WebGPU rather than native DirectX. |
...
- `zh_ww3d2_compile_frontier`: 105 original `WWVegas/WW3D2`
  ...
  text-texture, DDS, D3D caps/buffer/renderer, shader/material/texture/surface
  bodies, render-info, debug, and utility leaves compiled into a wasm static
  library as the current renderer-library compile frontier.
