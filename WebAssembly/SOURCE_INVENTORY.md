...
| `GameEngine/GameClient` | Partial | Utility slice compiles to wasm from original source including display-string/font/text/image/animation/weather/water/video utilities, original view/camera state, `GameClientDispatch.cpp`, map/terrain utility compile coverage (`MapUtil.cpp`, `RadiusDecal.cpp`, `TerrainRoads.cpp`, `TerrainVisual.cpp`), campaign manager compile coverage, hint/ray-effect compile coverage (`HintSpy.cpp`, `RayEffect.cpp`), particle system compile coverage (`ParticleSys.cpp`), drawable update compile coverage (`BeaconClientUpdate.cpp`, `SwayClientUpdate.cpp`), message/input translation compile coverage (`GUICommandTranslator.cpp`, `HotKey.cpp`, `LookAtXlat.cpp`, `MetaEvent.cpp`, `PlaceEventTranslator.cpp`, `SelectionXlat.cpp`, `WindowXlat.cpp`), `Eva.cpp`, `FXList.cpp`, `SelectionInfo.cpp`, window/layout/transition managers, `GUI/GameWindow.cpp`, core gadgets, selected GUI callbacks/menus including in-game chat, in-game popup, keyboard options, skirmish map/game options, LAN map/options/lobby, game info, map select, quit, disconnect/connection windows and menu containers, generals experience points, control-bar callbacks/popup descriptions, and replay controls, leaf ControlBar helpers/schemes/beacon/multiselect/OCL/observer/structure-inventory/under-construction sources, and GameClient-facing INI leaf parser sources (`INIAnimation.cpp`, `INICommandButton.cpp`, `INIMappedImage.cpp`, `INIVideo.cpp`, `INIWater.cpp`). Node smoke coverage verifies the currently linked non-rendering utility behavior; campaign/view/window/GUI/style/gadget/parser/map/terrain/radius-decal/hint/ray-effect/particle/drawable-update/message-translator/EVA/FX/menu/control-bar files are compile coverage only until `ControlBar`, `InGameUI`, drawable/display, terrain, input, real Xfer/INI/GameLogic/GlobalData, replay state, GameInfo/LAN, and the browser render/video layers link and are harness-driven. |
...
- `zh_gameclient_utility`: original `GameEngine/GameClient` utility sources
  ...
  in-game popup, game-info, map-select, quit, and control-bar callback
  leaves, drawable beacon/sway client updates, `GameClientDispatch.cpp`,
  `Eva.cpp`, `FXList.cpp`,
...
5. Continue `GameEngine/GameClient` upward from the current utility slice into
   terrain, input, shell/menu, drawable/display surfaces, and real video
   playback only as the corresponding original INI/Xfer/GlobalData/GameLogic
   and browser device contracts are available.
