...
that path. It is source-only for the full load/score ownership graph, but it now
also pins the focused browser Bink/W3D runtime proof that links original
`ScoreScreen.cpp`, installs a gated harness-only blank-layout hook, and drives
original `PlayMovieAndBlock("VS_small")` for 70 decoded frame presentations
...
then drives original `ChallengeLoadScreen::init` through a focused real
`CampaignManager` / `ChallengeGenerals` campaign/persona setup: the campaign
selects the player persona, the mission selects the opponent persona and
`GC_Background` movie label, static-text bio entries render through the original
teletype path, and persona/ambient audio events are asserted across 551
Challenge presentations. Full campaign-owned load-screen setup, full production
Challenge persona setup from the normal shell/INI path, full non-test
`finishSinglePlayerInit` subsystem ownership, production
...
- `zh_miles_audio_device_compile_frontier`: original
  ...
  startup/provider/listener/filter/sample/stream-handle boundaries. `mssStartupProbe` is the
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  original `ChallengeLoadScreen::init` campaign/persona ownership, while native `.bik` frame
  decoding, audio sync, full WebCodecs/`<video>` runtime ownership,
  campaign-owned load-screen setup, full production Challenge persona setup from the normal shell/INI path, and full
