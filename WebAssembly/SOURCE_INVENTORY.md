...
| `GameEngineDevice/VideoDevice` | Partial | Original `VideoDevice/Bink/BinkVideoPlayer.cpp` now compiles to wasm as `zh_bink_video_device_compile_frontier` against the real `GameClient/VideoPlayer` interfaces and a narrow compile-only Bink declaration shim for the handle fields, surface constants, and API calls used by the original player and stream code. `npm run verify:bink-video-device-frontier` pins the exact current source frontier: `BinkVideoPlayer::init` (line 128) calling `VideoPlayer::init()` (131) then `initializeBinkWithMiles()` (133); `deinit` (140) calling `releaseHandleForBink()` (142) then `VideoPlayer::deinit()` (143); `open` (221) using `BinkOpen` on the mod (233), localized (243), and fallback (249) paths then `createStream`; `createStream` (187) setting `m_handle` (200) and `BinkSetVolume` (210); `initializeBinkWithMiles` (283) calling `getHandleForBink` (286), `BinkSoundUseDirectSound` (290), and `BinkSetSoundTrack` (294); the `BinkVideoStream` destructor closing the handle via `BinkClose` (316); and `update`/`isFrameReady`/`frameDecompress`/`frameRender`/`frameNext`/`frameGoto`/`height`/`width`/`frameIndex`/`frameCount` mapping to `BinkWait`/`BinkDoFrame`/`BinkCopyToBuffer`/`BinkNextFrame`/`BinkGoto`/handle `Height`/`Width`/`FrameNum`/`Frames` fields. The shim `WebAssembly/shims/bink.h` declares the BINK struct, HBINK, surface/preload constants, and the Bink API signatures but is declarations only. `extract_zh_runtime_archives.sh` preserves the current shipped loose Bink payloads from `Data1.cab` (`GC_Background.bik`, 149,700 bytes, and `VS_small.bik`, 310,128 bytes) as ignored real-assets artifacts with BIK/KB2 header checks; these files are not BIG entries. `npm run inventory:bink-video-payloads` is the paired real-asset preflight: it indexes the current BIG directory set plus any loose `.bik` files already present under the assets dir, sniffs a small header prefix from each Bink entry, classifies classic `BIK` and Bink 2 `KB2` signatures without decoding video, reports `Data1.cab` presence and `looseBikExtractionRequired`, and pins that current Zero Hour BIG archives contain zero `.bik` entries while the expected loose set is either not yet extracted or exactly `GC_Background.bik` and `VS_small.bik`. `npm run verify:bink-payload-header-contract` (`WebAssembly/tools/verify_bink_payload_header_contract.mjs`) is a bounded real-data verifier that parses the actual extracted `GC_Background.bik` and `VS_small.bik` files (failing with a clear `npm run extract:runtime-archives` message if absent) and reads only the source-grounded classic-BINK header fields visible in the real files: `BIK` magic + version (BIKi), the u32 size field (file size minus 8), frame count, largest frame size, repeated frame count field, width, height, and fps numerator/denominator. Under `--expect-current-zh` it pins the measured values: `GC_Background.bik` 149,700 bytes, BIKi, 180 frames, 800x600, 30/1 fps; `VS_small.bik` 310,128 bytes, BIKi, 71 frames, 96x120, 30/1 fps; and verifies the size field equals file size minus 8. It emits JSON `{ ok, errors, payloads, source }` and does not decode/demux/play video. This is compile/extraction/inventory/header coverage only: the Bink API declarations do not decode `.bik` files, upload video frames to browser textures, drive audio synchronization, or provide a WebCodecs/`<video>` runtime; runtime WebCodecs/video decode remains open. |
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  API declarations. This covers the original Bink video player at compile time
  only; `.bik` decoding, frame upload, audio sync, WebCodecs/`<video>` runtime,
  and harness validation remain open. The runtime archive extractor now pulls
  the shipped loose `GC_Background.bik` and `VS_small.bik` files from
  `Data1.cab` and checks their BIK/KB2 header magic, so future runtime work can
  target real extracted video payloads instead of only source declarations.
  `npm run
  verify:bink-video-device-frontier` pins the exact current source frontier
  (player `init`/`deinit`/`open`/`createStream`/`initializeBinkWithMiles` order,
  stream lifecycle/decompress/render/advance/goto/dimension calls, header
  declarations, and the declarations-only `shims/bink.h`) and exits nonzero if
  any hard fact moves.
