...
| `GameEngine/GameClient` | Partial | Utility slice compiles to wasm from original source including display-string/font/text/image/animation/weather/water/video utilities, original view/camera state, `GameClientDispatch.cpp`, map/terrain utility compile coverage (`MapUtil.cpp`, `RadiusDecal.cpp`, `TerrainRoads.cpp`), campaign manager compile coverage, hint/ray-effect compile coverage (`HintSpy.cpp`, `RayEffect.cpp`), message/input translation compile coverage (`GUICommandTranslator.cpp`, `HotKey.cpp`, `LookAtXlat.cpp`, `MetaEvent.cpp`, `PlaceEventTranslator.cpp`, `SelectionXlat.cpp`, `WindowXlat.cpp`), `Eva.cpp`, `FXList.cpp`, `SelectionInfo.cpp`, window/layout/transition managers, `GUI/GameWindow.cpp`, core gadgets, selected GUI callbacks/menus including in-game chat, keyboard options, skirmish map/game options, LAN map/options/lobby, disconnect/connection windows, generals experience points, and replay controls, leaf ControlBar helpers/schemes/beacon/multiselect/OCL/observer/structure-inventory/under-construction sources, and GameClient-facing INI leaf parser sources (`INIAnimation.cpp`, `INICommandButton.cpp`, `INIMappedImage.cpp`, `INIVideo.cpp`, `INIWater.cpp`). Node smoke coverage verifies the currently linked non-rendering utility behavior; campaign/view/window/GUI/style/gadget/parser/map/terrain/radius-decal/hint/ray-effect/message-translator/EVA/FX/menu/control-bar files are compile coverage only until `ControlBar`, `InGameUI`, drawable/display, terrain, input, real Xfer/INI/GameLogic/GlobalData, replay state, GameInfo/LAN, and the browser render/video layers link and are harness-driven. |
| `GameEngine/GameNetwork` | Partial | Core command/frame and setup/LAN slice now compiles to wasm from original source: `Connection.cpp`, `DisconnectManager.cpp`, `FileTransfer.cpp`, `FirewallHelper.cpp`, `FrameData.cpp`, `FrameDataManager.cpp`, `FrameMetrics.cpp`, `GameInfo.cpp`, `GameMessageParser.cpp`, `GameSpy/GSConfig.cpp`, `GUIUtil.cpp`, `LANAPICallbacks.cpp`, `LANAPIhandlers.cpp`, `LANGameInfo.cpp`, `NetCommandList.cpp`, `NetCommandMsg.cpp`, `NetCommandRef.cpp`, `NetCommandWrapperList.cpp`, `NetMessageStream.cpp`, `NetPacket.cpp`, `NetworkUtil.cpp`, and `User.cpp`, plus original `Common/MessageStream.cpp` in the Common archive. Node smoke coverage verifies command-id generation, command type policy/name lookups, empty frame readiness through `FrameData`/`FrameDataManager`, and pooled `User` value behavior. `Connection.cpp`, `DisconnectManager.cpp`, `FileTransfer.cpp`, `FirewallHelper.cpp`, `FrameMetrics.cpp`, `GameInfo.cpp`, `GUIUtil.cpp`, `LANAPICallbacks.cpp`, `LANAPIhandlers.cpp`, `LANGameInfo.cpp`, `NetCommandMsg.cpp`, `NetMessageStream.cpp`, and `NetPacket.cpp` are compile coverage only until browser transport, `Shell`, `LoadScreen`, `Display`, real `GlobalData`, `MapCache`, multiplayer/player-template/game-text singletons, player/message dependencies, disconnect UI, LAN callback flow, and full packet/setup serialization smokes are available. WinSock transport, GameSpy Peer/WWDownload/NAT, LAN UI, and runtime packet flow remain open. |
...
- `zh_gameclient_utility`: original `GameEngine/GameClient` utility sources
  ...
  transition sources, `GameClientDispatch.cpp`, `Eva.cpp`, `FXList.cpp`,
...
- `zh_gamenetwork_core`: original `GameEngine/GameNetwork` command/frame,
  ...
  `DisconnectManager.cpp`, `FileTransfer.cpp`, `FirewallHelper.cpp`,
  `FrameData.cpp`, `FrameDataManager.cpp`, `FrameMetrics.cpp`,
  `GameInfo.cpp`, `GameMessageParser.cpp`, `GameSpy/GSConfig.cpp`,
  `GUIUtil.cpp`, `LANAPICallbacks.cpp`, `LANAPIhandlers.cpp`,
  `LANGameInfo.cpp`,
