...
| `GameEngineDevice/W3DDevice` | Partial | Original `W3DDevice/Common/W3DConvert.cpp` now compiles in the real-header frontier as a pure coordinate-conversion utility. The W3D display, scene, terrain, drawable, shader, water, shadow, and GUI device implementations remain renderer work and are not runtime-complete until re-targeted to WebGL2/WebGPU and verified through harness screenshots. |
...
- `zh_gameengine_real_compile_frontier`: compile-only original
  ...
  device common leaves (`W3DDevice/Common/W3DConvert.cpp`,
