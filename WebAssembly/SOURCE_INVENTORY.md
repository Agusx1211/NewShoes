...
| `GameEngine/GameClient` | Partial | Utility slice compiles to wasm from 139 of 147 original sources, including display-string/font/text/image/animation/weather/water/video utilities, original top-level `GameClient.cpp`, `InGameUI.cpp`, `Input/Mouse.cpp`, `GUI/IMEManager.cpp`, original view/camera state, `GameClientDispatch.cpp`, drawable core/smudge/audio debug leaves (`Drawable.cpp`, `Drawable/DrawableManager.cpp`, `System/Smudge.cpp`, `System/Debug Displayers/AudioDebugDisplay.cpp`), map/terrain utility compile coverage (`MapUtil.cpp`, `RadiusDecal.cpp`, `TerrainRoads.cpp`, `TerrainVisual.cpp`), campaign manager compile coverage, hint/ray-effect compile coverage (`HintSpy.cpp`, `RayEffect.cpp`), particle system compile coverage (`ParticleSys.cpp`), drawable update compile coverage (`AnimatedParticleSysBoneClientUpdate.cpp`, `BeaconClientUpdate.cpp`, `SwayClientUpdate.cpp`), message/input translation compile coverage (`CommandXlat.cpp`, `GUICommandTranslator.cpp`, `HotKey.cpp`, `LookAtXlat.cpp`, `MetaEvent.cpp`, `PlaceEventTranslator.cpp`, `SelectionXlat.cpp`, `WindowXlat.cpp`), `Eva.cpp`, `FXList.cpp`, `SelectionInfo.cpp`, window/layout/transition managers, `GUI/GameWindow.cpp`, `GUI/LoadScreen.cpp`, `GUI/Shell/Shell.cpp`, core gadgets, selected GUI callbacks/menus including diplomacy, in-game chat, in-game popup, keyboard options, skirmish map/game options, LAN map/options/lobby, challenge/difficulty, game info, map select, network direct-connect, host/join/player-info/ladder popups, score screen, quit, replay/save-load/export, WOL buddy/lobby/locale/map/quick-match leaves, disconnect/connection windows and menu containers, generals experience points, control-bar core/command/callback/popup descriptions, replay controls, leaf ControlBar helpers/schemes/beacon/multiselect/OCL/observer/structure-inventory/under-construction sources, and GameClient-facing INI leaf parser sources (`INIAnimation.cpp`, `INICommandButton.cpp`, `INIMappedImage.cpp`, `INIVideo.cpp`, `INIWater.cpp`). Node smoke coverage verifies the currently linked non-rendering utility behavior; campaign/view/window/GUI/style/gadget/parser/map/terrain/radius-decal/hint/ray-effect/particle/drawable/drawable-update/smudge/audio-debug/message-translator/EVA/FX/menu/control-bar/top-level-client/InGameUI/mouse/IME/replay/load/shell/WOL files are compile coverage only until original ControlBar behavior, drawable/display, terrain, real browser input/IME, real Xfer/INI/GameLogic/GlobalData, replay state, GameInfo/LAN/GameSpy, and the browser render/video layers link and are harness-driven. The remaining sources are `DownloadMenu.cpp`, `MainMenu.cpp`, `OptionsMenu.cpp`, `WOLGameSetupMenu.cpp`, `WOLLadderScreen.cpp`, `WOLLoginMenu.cpp`, `WOLWelcomeMenu.cpp`, and `Input/Keyboard.cpp`. |
| `GameEngine/GameNetwork` | Partial | Core command/frame and setup/LAN slice now compiles to wasm from original source: `Connection.cpp`, `ConnectionManager.cpp`, `DisconnectManager.cpp`, `FileTransfer.cpp`, `FirewallHelper.cpp`, `FrameData.cpp`, `FrameDataManager.cpp`, `FrameMetrics.cpp`, `GameInfo.cpp`, `GameMessageParser.cpp`, `GameSpy/GSConfig.cpp`, `GUIUtil.cpp`, `LANAPI.cpp`, `LANAPICallbacks.cpp`, `LANAPIhandlers.cpp`, `LANGameInfo.cpp`, `NetCommandList.cpp`, `NetCommandMsg.cpp`, `NetCommandRef.cpp`, `NetCommandWrapperList.cpp`, `NetMessageStream.cpp`, `NetPacket.cpp`, `NetworkUtil.cpp`, and `User.cpp`, plus original `Common/MessageStream.cpp` in the Common archive. The real-header compile frontier also covers original `GameSpy/Chat.cpp`, `GameSpy/LadderDefs.cpp`, `GameSpy/PeerDefs.cpp`, `GameSpy/StagingRoomGameInfo.cpp`, GameSpy thread bodies (`BuddyThread.cpp`, `GameResultsThread.cpp`, `PeerThread.cpp`, `PersistentStorageThread.cpp`, `PingThread.cpp`, and `ThreadUtils.cpp`), `GameSpyOverlay.cpp`, `IPEnumeration.cpp`, `NAT.cpp`, `Network.cpp`, `Transport.cpp`, and `udp.cpp`. Node smoke coverage verifies command-id generation, command type policy/name lookups, empty frame readiness through `FrameData`/`FrameDataManager`, and pooled `User` value behavior. `Connection.cpp`, `ConnectionManager.cpp`, `DisconnectManager.cpp`, `FileTransfer.cpp`, `FirewallHelper.cpp`, `FrameMetrics.cpp`, `GameInfo.cpp`, `GUIUtil.cpp`, `LANAPI.cpp`, `LANAPICallbacks.cpp`, `LANAPIhandlers.cpp`, `LANGameInfo.cpp`, `NetCommandMsg.cpp`, `NetMessageStream.cpp`, `NetPacket.cpp`, and the real-header GameSpy/transport/NAT/thread additions are compile coverage only until browser transport, `Shell`, `LoadScreen`, `Display`, real `GlobalData`, `GameLogic` progress/game-start state, `MapCache`, multiplayer/player-template/game-text singletons, player/message dependencies, disconnect UI, LAN callback flow, browser Worker/pthread scheduling, GameSpy GP/Peer/QR2/Stats runtime queues, ICMP/socket fallbacks, and full packet/setup serialization smokes are available. WWDownload, LAN UI, and runtime packet flow remain open. |
...
| `GameSpy` | Partial | Declarative Peer/GP/QR2/persistent-storage/SNMP compile shims now cover the original GameEngine GameSpy definitions, preference/recorder include path, and thread source compile surface. Runtime online/networking behavior, thread queues, stats APIs, ICMP/socket fallbacks, and browser relay/WebRTC mapping remain open. |
...
- `zh_gameengine_real_compile_frontier`: compile-only original
  ...
  leaf/parser sources except `INIWebpageURL.cpp`, RTS compile leaves
  ...
  `GameSpy/LadderDefs.cpp`, `GameSpy/PeerDefs.cpp`,
...
- `zh_gameclient_utility`: original `GameEngine/GameClient` utility sources
  ...
  GUI/window manager and selected gadget/callback/control-bar/menu/replay/
  transition sources, `GUI/LoadScreen.cpp`, `GUI/Shell/Shell.cpp`,
  disconnect/connection menu containers, diplomacy, network direct-connect,
  host/join/player-info/ladder popups, score screen, WOL buddy/lobby/locale/
  map/quick-match leaves, in-game popup, game-info, map-select, quit,
  challenge/difficulty, replay save/load/export, control-bar core/command
  sources, and
  control-bar callback leaves, drawable
  animated-particle/beacon/sway client updates, `GameClient.cpp`,
  `InGameUI.cpp`, `Input/Mouse.cpp`, `GUI/IMEManager.cpp`,
...
- `zh_gamenetwork_core`: original `GameEngine/GameNetwork` command/frame,
  ...
  `ConnectionManager.cpp`, `DisconnectManager.cpp`, `FileTransfer.cpp`,
  `FirewallHelper.cpp`,
  ...
  `NetworkUtil.cpp`, and `User.cpp` compiled into a wasm static library.
...
- `gamenetwork-core-smoke`: a Node-executed wasm smoke test that verifies
  ...
  `User` value semantics. `Connection.cpp`, `FileTransfer.cpp`,
  `NetMessageStream.cpp`, and `NetPacket.cpp` are currently compile coverage
  only.
