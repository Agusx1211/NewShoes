...
| `WWVegas/WW3D2` | Partial | `zh_ww3d2_compile_frontier` now builds 55 original non-Direct3D WW3D2 sources: hierarchy/animation/LOD and render-object definitions (`animobj.cpp`, `agg_def.cpp`, `hanim.cpp`, `hanimmgr.cpp`, `hcanim.cpp`, `hlod.cpp`, `hmorphanim.cpp`, `hmdldef.cpp`, `hrawanim.cpp`, `htree.cpp`, `htreemgr.cpp`, `distlod.cpp`, `composite.cpp`, `nullrobj.cpp`, `rendobj.cpp`, `soundrobj.cpp`, `renderobjectrecycler.cpp`), spatial/collision/visibility helpers (`aabtree.cpp`, `aabtreebuilder.cpp`, `coltest.cpp`, `intersec.cpp`, `meshgeometry.cpp`, `proto.cpp`, `visrasterizer.cpp`, `bw_render.cpp`, `bwrender.cpp`), light/projector/decal/particle/snap-point support (`light.cpp`, `projector.cpp`, `decalsys.cpp`, `part_emt.cpp`, `part_ldr.cpp`, `snappts.cpp`), asset/status/cache/exclusion and W3D dependency helpers (`assetstatus.cpp`, `texfcach.cpp`, `texturefile.cpp`, `w3d_dep.cpp`, `w3d_util.cpp`, `w3dexclusionlist.cpp`), mesh/material/metadata and algorithm helpers (`matinfo.cpp`, `meshbuild.cpp`, `meshdam.cpp`, `metalmap.cpp`, `motchan.cpp`, `polyinfo.cpp`, `predlod.cpp`, `prim_anim.cpp`, `stripoptimizer.cpp`), and small support/debug/text files (`dx8rendererdebugger.cpp`, `font3d.cpp`, `layer.cpp`, `pivot.cpp`, `pot.cpp`, `rinfo.cpp`, `txt.cpp`). This is compile coverage only; DirectX 8 renderer bodies, scene/camera runtime, texture/material/shader systems, GDI text rendering, Video-for-Windows frame grabbing, Miles/WWAudio binding, Surrender `sr*` utility headers, and render-object/light runtime linking still need browser ports before WW3D2 can be runtime-complete. |
...
`lib/BaseType.h`, `lib/baseType.h`, `convert.h`, `font.h`,
...
type, W3D light enum, WWLib font/convert declarations, W3D streak-render
declarations, Vector3i, and Matrix3D inline behavior; Windows Media / shell URL
...
- `zh_ww3d2_compile_frontier`: 55 original non-Direct3D `WWVegas/WW3D2`
  hierarchy/animation/LOD, render-object definition, spatial/collision,
  visibility, light/projector/decal/particle, snap-point, asset/cache,
  texture-file metadata, material metadata, primitive animation, text-texture,
  render-info, debug, and utility leaves compiled into a wasm static library
  as the current renderer-library compile frontier.
