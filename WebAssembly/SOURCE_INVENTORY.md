...
| `WWVegas/WW3D2` | Partial | A first non-Direct3D compile-frontier batch now builds as `zh_ww3d2_compile_frontier` from original sources: animation/render-object helpers (`animobj.cpp`, `composite.cpp`, `nullrobj.cpp`, `soundrobj.cpp`, `renderobjectrecycler.cpp`), collision/intersection and render-info helpers (`coltest.cpp`, `intersec.cpp`, `rinfo.cpp`), light/projector and W3D utility/dependency paths (`light.cpp`, `projector.cpp`, `w3d_dep.cpp`, `w3d_util.cpp`), asset/status/cache/exclusion helpers (`assetstatus.cpp`, `texfcach.cpp`, `w3dexclusionlist.cpp`), metadata/container helpers (`meshdam.cpp`, `polyinfo.cpp`, `predlod.cpp`), and small support files (`font3d.cpp`, `layer.cpp`, `pivot.cpp`, `pot.cpp`). This is compile coverage only; DirectX 8 renderer bodies, scene/camera runtime, texture/material/shader systems, GDI text handles, mesh geometry include gaps, and render-object/light linking still need browser ports before WW3D2 can be runtime-complete. |
...
- `zh_ww3d2_compile_frontier`: original non-Direct3D `WWVegas/WW3D2`
  animation, collision/intersection, light/projector, asset/cache, metadata,
  render-info, sound render-object, and utility leaves compiled into a wasm
  static library as the first renderer-library compile frontier.
