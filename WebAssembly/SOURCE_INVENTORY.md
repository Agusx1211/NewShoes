...
also exposes `browserAudioRequestPathRuntime`, which takes one real decoded
requested payload through a source-shaped `AudioManager::addAudioEvent` handle,
filename, play-info, `SoundManager::addAudioEvent` `AR_Play` queue,
`MilesAudioManager::processRequestList` / `processRequest` dispatch, and live
`AudioBufferSourceNode` completion/release sequence. This is still
...
- `zh_miles_audio_device_compile_frontier`: original
  ...
  game audio manager through a stateful startup probe only; Web Audio playback,
  stream decoding, sample/stream ownership, 3D positioning, Bink audio handoff,
  and harness-observable engine playback remain open. `npm run
  ...
  startup/provider/listener/sample-handle boundary. `mssStartupProbe` is the
  ...
  playback unready. The playback-handle companion verifier `npm
  ...
  compile-only/inert, not runtime audio. `npm run
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  and harness validation remain open. `npm run
