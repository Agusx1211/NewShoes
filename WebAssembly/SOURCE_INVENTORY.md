...
| `GameEngineDevice/W3DDevice` | Partial | Original `W3DDevice/Common/W3DConvert.cpp`, `Common/Thing/W3DThingFactory.cpp`, `GameLogic/W3DGameLogic.cpp`, and `GameClient/TileData.cpp` now compile in the real-header frontier as lightweight device/factory and terrain tile-data coverage. `WorldHeightMap.h` has been made self-contained for its map-object and terrain-texture pointer declarations. The W3D module factory, function lexicon, height maps, display, scene, terrain renderer, drawable modules, shader, water, shadow, and GUI device implementations remain renderer work and are not runtime-complete until Direct3D/WW3D2 surfaces are re-targeted to WebGL2/WebGPU and verified through harness screenshots. |
...
`WWLib/RefCount.h`, `WWMATH/Vector2.h`, `WWMATH/Vector3.h`, and
