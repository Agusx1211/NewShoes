...
- `zh_miles_audio_device_compile_frontier`: original
  ...
  validation remain open. `npm run verify:miles-audio-device-frontier` is the
  source verifier for this frontier and pins the exact current device-startup
  facts: `MilesAudioManager::init` line 444, `AudioManager::init` call line
