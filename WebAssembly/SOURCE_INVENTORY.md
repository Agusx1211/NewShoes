...
| `GameEngineDevice/W3DDevice` | Partial | Original `W3DDevice/Common/W3DConvert.cpp`, `Common/Thing/W3DThingFactory.cpp`, `GameLogic/W3DGameLogic.cpp`, `GameClient/TileData.cpp`, `GameClient/W3DPoly.cpp`, and `GameClient/W3DFileSystem.cpp` now compile in the real-header frontier as lightweight device/factory, terrain tile-data, polygon clipping, and W3D file-factory coverage. `zh_w3d_device_utility` link-smokes original `TileData.cpp` mip generation and original `W3DPoly.cpp` clipping behavior under wasm. `WorldHeightMap.h` has been made self-contained for its map-object and terrain-texture pointer declarations, and `WWLIB/ffactory.h` is bridged for the original W3D file-factory header casing. `W3DFileSystem.cpp` remains compile coverage only until the browser asset `FileSystem`, user-data path, and WWLib file-factory singleton contract can be linked and smoked. The W3D module factory, function lexicon, height maps, display, scene, terrain renderer, drawable modules, shader, water, shadow, and GUI device implementations remain renderer work and are not runtime-complete until Direct3D/WW3D2 surfaces are re-targeted to WebGL2/WebGPU and verified through harness screenshots. |
