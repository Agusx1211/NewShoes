...
| `DX90SDK` | Partial | Target-local `D3DX8Math.h` shim covers vector3/vector4/matrix operations and D3DX-style float-pointer conversions used by original `GameEngine/Common` Bezier helpers, original `WWMath/matrix3d.cpp`, and the current WW3D2/wwshade sorting, point-group, light, and shader paths; lowercase/case-variant wrappers handle original `D3dx8math.h`, `d3dx8math.h`, `d3dx8.h`, `D3dx8core.h`, `d3dx8core.h`, `D3dx8tex.h`, and `d3dx8tex.h` include spellings on case-sensitive filesystems. Declaration-only `d3d8.h`, `d3d8types.h`, `d3d8caps.h`, and `ddraw.h` surfaces now cover Direct3D formats, FVF helpers, DX8 vertex-shader declaration tokens, COM interface declarations plus legacy `LPDIRECT3D*8` pointer aliases, DirectDraw DDS caps, D3DX texture creation/load/filter declarations, D3DX shader-buffer/assembly declarations, and constants needed by the current WW3D2, wwshade, and W3D terrain-support compile frontiers. Broader DirectX 8 runtime compatibility for WW3D and W3D device code remains open and must map to WebGL2/WebGPU rather than native DirectX. |
...
`WWMath/Matrix3D.h`,
`WWLIB/ffactory.h`, `WWLib/RefCount.h`, `Texture.h`,
...
headers, D3D/D3DX format/FVF/texture helpers, W3D collision type, W3D light
enum, representative W3D2 asset/render/texture bridge headers, WWLib
font/convert declarations, W3D height-map bridge headers, W3D streak-render
declarations, Vector3i, and Matrix3D inline behavior; the full W3D bridge batch
