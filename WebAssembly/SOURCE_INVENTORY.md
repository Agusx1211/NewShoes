...
| `GameEngineDevice/MilesAudioDevice` | Partial | Original `MilesAudioManager.cpp` now compiles to wasm as `zh_miles_audio_device_compile_frontier` against real game/audio headers and the current compile-only Miles, DirectSound speaker-config/Bink-handle, and Win32 mutex declaration surface. This is compile coverage only: the `Mss.H` functions are inert, `dsound.h` does not expose a browser device, and playback, sample/stream handle ownership, ADPCM/stream decoding, mixer volume, 3D positioning, Bink audio sharing, and Web Audio scheduling still need a real browser implementation plus harness-observable validation. |
| `GameEngineDevice/VideoDevice` | Partial | Original `VideoDevice/Bink/BinkVideoPlayer.cpp` now compiles to wasm as `zh_bink_video_device_compile_frontier` against the real `GameClient/VideoPlayer` interfaces and a narrow compile-only Bink declaration shim for the handle fields, surface constants, and API calls used by the original player and stream code. This is compile coverage only: the Bink API declarations do not decode `.bik` files, upload video frames to browser textures, drive audio synchronization, or provide a WebCodecs/`<video>` runtime. |
...
- `zh_miles_audio_device_compile_frontier`: original
  ...
  a wasm static library against real game/audio headers and compile-only
  Miles/DirectSound/Win32 compatibility declarations. This covers the original
  game audio manager at compile time only; Web Audio playback, stream decoding,
  sample/stream ownership, 3D positioning, Bink audio handoff, and harness
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  and harness validation remain open.
