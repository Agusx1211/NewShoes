...
| `GameSpy` | Partial | Declarative Peer/GP/persistent-storage/SNMP compile shims now cover the original GameEngine GameSpy definitions and preference/recorder include path. Runtime online/networking behavior, thread queues, stats APIs, and browser relay/WebRTC mapping remain open. |
...
- `zh_gameengine_real_compile_frontier`: compile-only original
  ...
  `GameSpy/StagingRoomGameInfo.cpp`, `GameSpy/Thread/ThreadUtils.cpp`,
...
- `zh_gameclient_utility`: original `GameEngine/GameClient` utility sources
  ...
  transition sources, disconnect/connection menu containers,
  in-game popup, game-info, map-select, quit, challenge/difficulty,
  replay save/load/export, control-bar core/command sources, and
  ...
  (`GUICommandTranslator.cpp`, `HintSpy.cpp`, `HotKey.cpp`, `LookAtXlat.cpp`,
