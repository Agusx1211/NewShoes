...
| `GameEngineDevice/VideoDevice` | Partial | Original `VideoDevice/Bink/BinkVideoPlayer.cpp` now compiles to wasm as `zh_bink_video_device_compile_frontier` against the real `GameClient/VideoPlayer` interfaces and the browser Bink API surface used by the original player and stream code. `npm run verify:bink-video-device-frontier` pins the exact current source frontier: `BinkVideoPlayer::init` (line 128) calling `VideoPlayer::init()` (131) then `initializeBinkWithMiles()` (133); `deinit` (140) calling `releaseHandleForBink()` (142) then `VideoPlayer::deinit()` (143); `open` (221) using `BinkOpen` on the mod (233), localized (243), and fallback (249) paths then `createStream`; `createStream` (187) setting `m_handle` (200) and `BinkSetVolume` (210); `initializeBinkWithMiles` (283) calling `getHandleForBink` (286), `BinkSoundUseDirectSound` (290), and `BinkSetSoundTrack` (294); the `BinkVideoStream` destructor closing the handle via `BinkClose` (316); and `update`/`isFrameReady`/`frameDecompress`/`frameRender`/`frameNext`/`frameGoto`/`height`/`width`/`frameIndex`/`frameCount` mapping to `BinkWait`/`BinkDoFrame`/`BinkCopyToBuffer`/`BinkNextFrame`/`BinkGoto`/handle `Height`/`Width`/`FrameNum`/`Frames` fields. The shim `WebAssembly/shims/bink.h` declares the BINK struct, HBINK, surface/preload constants, and the original Bink API signatures; `WebAssembly/src/wasm_bink_provider.cpp` now supplies the first browser provider definitions for real-file open/header parsing, original-style basename resolution into `artifacts/real-assets`, handle field population, sound-track/volume calls, and frame-cursor lifecycle. `test:bink-video-provider` builds and runs a wasm smoke against the real shipped loose `GC_Background.bik` and `VS_small.bik` payloads, proving `180` frames at `800x600` and `71` frames at `96x120` with `decodeReady:false`. `npm run verify:bink-payload-header-contract` parses those same extracted files and pins the source-grounded classic-BINK header fields: BIKi magic/version, size field equal to file size minus 8, frame count, largest frame size, repeated frame count, width, height, and 30/1 fps. `extract_zh_runtime_archives.sh` preserves those files from `Data1.cab` as ignored real-assets artifacts with BIK/KB2 header checks; they are not BIG entries. `npm run inventory:bink-video-payloads` is the paired real-asset preflight and pins that current Zero Hour BIG archives contain zero `.bik` entries while the expected loose set is either not yet extracted or exactly those two files. This is compile/extraction/inventory/header/open lifecycle coverage only: the provider does not decode `.bik` frames, upload video frames to browser textures, drive audio synchronization, or provide a WebCodecs/`<video>` runtime; runtime WebCodecs/video decode remains open. `npm run verify:bink-browser-video-outputs` (with `:strict` variant) is the bounded verifier for whatever future browser-decodable transcode outputs that work produces: it validates a future output directory for the two shipped loose payloads against a tight source-grounded manifest schema, verifies real width/height/codec/duration via `ffprobe` when available, and with `--allow-missing` reports absent outputs in JSON without failing (useful before any transcode exists); it does not decode Bink video or claim runtime playback. |
