...
through `TheDisplay->draw()`. The same focused proof now compiles original
...
campaign `finishSinglePlayerInit` ownership, campaign score-screen ownership,
and the broader CampaignManager/GameInfo/GameWindow layout/LOD/shell/GUI
singleton surface remain open until they can be linked and harness-driven.
...
- `zh_bink_video_device_compile_frontier`: original
  ...
  blank-window first-window `setVideoBuffer`, focused original
  `SinglePlayerLoadScreen::init`, and focused original `ChallengeLoadScreen::init`
  ownership, while native `.bik` frame decoding, audio sync, full
  WebCodecs/`<video>` runtime ownership, and campaign-owned load-screen /
  Challenge persona / score-screen playback remain open. The runtime archive extractor
