...
- `zh_gameengine_real_compile_frontier`: compile-only original
  ...
  (`AcademyStats.cpp`, `ActionManager.cpp`, `ResourceGatheringManager.cpp`,
...
- `zh_gameclient_utility`: original `GameEngine/GameClient` utility sources
  ...
  replay save/load/export, and control-bar callback leaves, drawable
  ...
  ControlBar leaves (`ControlBarBeacon.cpp`, `ControlBarMultiSelect.cpp`,
  `ControlBarOCLTimer.cpp`, `ControlBarObserver.cpp`,
...
- `gameclient-utility-smoke`: a Node-executed wasm smoke test that verifies
  ...
  `View.cpp` currently add compile/link coverage only; their runtime behavior
