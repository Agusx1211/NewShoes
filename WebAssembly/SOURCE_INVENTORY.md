...
| `WWVegas/WWAudio` | Partial | `zh_wwaudio_compile_frontier` now builds all 19 original WWAudio sources: audible/logical/listener/sound scene objects, 2D/3D/pseudo/filtered sound classes, sound-buffer/handle/stream-handle helpers, save/load, utilities, delayed-release thread plumbing, and `WWAudio.cpp`. This is compile coverage only. The current `Mss.H` Miles surface has stateful startup, 2D sample, and stream lifecycle probes but still does not decode, schedule, or play audio; Win32 event/thread fallbacks are compile-only, and real playback, decoding, scheduling, 3D positioning, cache management, and Web Audio integration remain open before WWAudio can be used at runtime. |
...
- `zh_miles_audio_device_compile_frontier`: original
  ...
  game audio manager through stateful startup, 2D sample lifecycle, and HSTREAM
  lifecycle probes; Web Audio playback, stream decoding, decoded sample/stream
  ownership, 3D positioning, Bink audio handoff, and harness-observable engine
  playback remain open. `npm run
  ...
  unready. The playback-handle companion verifier `npm
