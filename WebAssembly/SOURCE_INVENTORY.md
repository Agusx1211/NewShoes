...
| `WWVegas/WW3D2` | Partial | `zh_ww3d2_compile_frontier` now builds 46 original non-Direct3D WW3D2 sources: hierarchy/animation/LOD and render-object definitions (`animobj.cpp`, `agg_def.cpp`, `hanimmgr.cpp`, `hlod.cpp`, `hmdldef.cpp`, `htree.cpp`, `htreemgr.cpp`, `distlod.cpp`, `composite.cpp`, `nullrobj.cpp`, `rendobj.cpp`, `soundrobj.cpp`, `renderobjectrecycler.cpp`), spatial/collision/visibility helpers (`aabtree.cpp`, `aabtreebuilder.cpp`, `coltest.cpp`, `intersec.cpp`, `meshgeometry.cpp`, `proto.cpp`, `visrasterizer.cpp`, `bw_render.cpp`, `bwrender.cpp`), light/projector/decal/particle/snap-point support (`light.cpp`, `projector.cpp`, `decalsys.cpp`, `part_emt.cpp`, `part_ldr.cpp`, `snappts.cpp`), asset/status/cache/exclusion and W3D dependency helpers (`assetstatus.cpp`, `texfcach.cpp`, `w3d_dep.cpp`, `w3d_util.cpp`, `w3dexclusionlist.cpp`), mesh/material/metadata and algorithm helpers (`meshbuild.cpp`, `meshdam.cpp`, `metalmap.cpp`, `motchan.cpp`, `polyinfo.cpp`, `predlod.cpp`, `stripoptimizer.cpp`), and small support files (`font3d.cpp`, `layer.cpp`, `pivot.cpp`, `pot.cpp`, `rinfo.cpp`). This is compile coverage only; DirectX 8 renderer bodies, scene/camera runtime, texture/material/shader systems, GDI text handles, Miles/WWAudio binding, and render-object/light runtime linking still need browser ports before WW3D2 can be runtime-complete. |
...
`lib/BaseType.h`, `lib/baseType.h`, `LightEnvironment.h`,
`hmdldef.h`, `snappts.h`, `vector3i.h`, `WW3D2/ColType.h`,
...
type, W3D light enum, Vector3i, and Matrix3D inline behavior; Windows Media /
...
- `zh_ww3d2_compile_frontier`: 46 original non-Direct3D `WWVegas/WW3D2`
  ...
  metadata, render-info, and utility leaves compiled into a wasm static
