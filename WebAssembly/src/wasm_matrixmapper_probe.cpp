...
std::string g_matrixmapper_probe_json;
...
EMSCRIPTEN_KEEPALIVE const char *cnc_port_probe_matrixmapper_apply()
{
	initMemoryManager();
	wasm_d3d8_reset_state();
	g_matrixmapper_probe_json.clear();

	constexpr unsigned kStage = 1;
	constexpr float kTextureSize = 128.0f;
	constexpr D3DTRANSFORMSTATETYPE kTextureTransformState =
		static_cast<D3DTRANSFORMSTATETYPE>(D3DTS_TEXTURE0 + kStage);
	const Matrix4x4 view_to_texture(
		1.00f, 0.10f, 0.20f, 0.30f,
		0.05f, 1.20f, 0.15f, 0.25f,
		0.02f, 0.07f, 0.90f, 0.40f,
		0.20f, 0.30f, 0.40f, 1.00f);
	...
	if (set_device_result == WW3D_ERROR_OK) {
		MatrixMapperClass *mapper = W3DNEW MatrixMapperClass(kStage);
		mapper_created = mapper != nullptr;

		Matrix4x4 expected_view_to_pixel;
		expected_view_to_pixel.Make_Identity();
		Matrix4x4 emitted_transform;
		emitted_transform.Make_Identity();
		if (mapper != nullptr) {
			mapper->Set_Type(MatrixMapperClass::PERSPECTIVE_PROJECTION);
			mapper->Set_Texture_Transform(view_to_texture, kTextureSize);
			mapper->Calculate_Texture_Matrix(expected_view_to_pixel);
		}
		...
		const bool row0_ok = row_nearly_equal(emitted_transform, 0, expected_view_to_pixel, 0);
		const bool row1_ok = row_nearly_equal(emitted_transform, 1, expected_view_to_pixel, 1);
		const bool row2_ok = row_nearly_equal(emitted_transform, 2, expected_view_to_pixel, 3);
		const bool ok =
			after_state != nullptr &&
			device != nullptr &&
			mapper_created &&
			...
			transform_flags == (D3DTTFF_PROJECTED | D3DTTFF_COUNT3) &&
		...
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"matrixmapper_apply_probe\","
			"\"ok\":%s,"
			...
			"\"perspectiveRowsOk\":%s,\"row0Ok\":%s,\"row1Ok\":%s,\"row2FromRow3Ok\":%s,"
			"\"row0\":[%.5f,%.5f,%.5f,%.5f],"
			"\"row1\":[%.5f,%.5f,%.5f,%.5f],"
			"\"row2\":[%.5f,%.5f,%.5f,%.5f],"
			"\"expectedRow0\":[%.5f,%.5f,%.5f,%.5f],"
			"\"expectedRow1\":[%.5f,%.5f,%.5f,%.5f],"
			"\"expectedSourceRow3\":[%.5f,%.5f,%.5f,%.5f]},"
			...
			static_cast<unsigned long>(D3DTTFF_PROJECTED | D3DTTFF_COUNT3),
			...
			bool_json(row0_ok && row1_ok && row2_ok),
			bool_json(row0_ok),
			bool_json(row1_ok),
			bool_json(row2_ok),
			...
			static_cast<double>(expected_view_to_pixel[0][0]),
			static_cast<double>(expected_view_to_pixel[0][1]),
			static_cast<double>(expected_view_to_pixel[0][2]),
			static_cast<double>(expected_view_to_pixel[0][3]),
			static_cast<double>(expected_view_to_pixel[1][0]),
			static_cast<double>(expected_view_to_pixel[1][1]),
			static_cast<double>(expected_view_to_pixel[1][2]),
			static_cast<double>(expected_view_to_pixel[1][3]),
			static_cast<double>(expected_view_to_pixel[3][0]),
			static_cast<double>(expected_view_to_pixel[3][1]),
			static_cast<double>(expected_view_to_pixel[3][2]),
			static_cast<double>(expected_view_to_pixel[3][3]),
			transform_calls_after - transform_calls_before,
			texture_stage_calls_after - texture_stage_calls_before);
		g_matrixmapper_probe_json = buffer;
	...
	if (g_matrixmapper_probe_json.empty()) {
		char buffer[512];
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"matrixmapper_apply_probe\",\"ok\":false,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d,"
			"\"stage\":%u,\"mapperCreated\":%s,\"applyCalled\":%s}}",
			init_result,
			set_device_result,
	...
	return g_matrixmapper_probe_json.c_str();
}
