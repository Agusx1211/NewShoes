...
std::string g_ws_environment_mapper_probe_json;
...
const char *axis_name(WSEnvMapperClass::AxisType axis)
{
	switch (axis) {
		case WSEnvMapperClass::AXISTYPE_X:
			return "X";
		case WSEnvMapperClass::AXISTYPE_Y:
			return "Y";
		case WSEnvMapperClass::AXISTYPE_Z:
		default:
			return "Z";
	}
}
...
TextureMapperClass *make_ws_mapper(bool reflection, WSEnvMapperClass::AxisType axis, unsigned stage)
{
	if (reflection) {
		return W3DNEW WSEnvironmentMapperClass(axis, stage);
	}
	return W3DNEW WSClassicEnvironmentMapperClass(axis, stage);
}

WSMapperCaseResult run_ws_mapper_case(
	const char *name,
	bool reflection,
	WSEnvMapperClass::AxisType axis,
	unsigned stage)
{
	WSMapperCaseResult result = {};
	result.name = name;
	...
	TextureMapperClass *mapper = make_ws_mapper(reflection, axis, stage);
...
bool case_ok(const WSMapperCaseResult &result)
{
...
void append_case_json(char *buffer, std::size_t buffer_size, const WSMapperCaseResult &result)
{
...
EMSCRIPTEN_KEEPALIVE const char *cnc_port_probe_ws_environment_mapper_apply()
{
	initMemoryManager();
	wasm_d3d8_reset_state();
	g_ws_environment_mapper_probe_json.clear();
	...
	if (set_device_result == WW3D_ERROR_OK) {
		WSMapperCaseResult classic = run_ws_mapper_case(
			"classic", false, WSEnvMapperClass::AXISTYPE_X, 1);
		WSMapperCaseResult reflection = run_ws_mapper_case(
			"reflection", true, WSEnvMapperClass::AXISTYPE_Y, 1);
		const bool ok = case_ok(classic) && case_ok(reflection);

		char classic_json[8192];
		char reflection_json[8192];
		...
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"ws_environment_mapper_apply_probe\","
			"\"ok\":%s,"
			...
			classic_json,
			reflection_json);
		g_ws_environment_mapper_probe_json = buffer;
	}
	...
	if (g_ws_environment_mapper_probe_json.empty()) {
		char buffer[256];
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"ws_environment_mapper_apply_probe\",\"ok\":false,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d}}",
			init_result,
			set_device_result);
		g_ws_environment_mapper_probe_json = buffer;
	}
	return g_ws_environment_mapper_probe_json.c_str();
}
