...
std::string g_classic_environment_mapper_probe_json;
...
EMSCRIPTEN_KEEPALIVE const char *cnc_port_probe_classic_environment_mapper_apply()
{
	initMemoryManager();
	wasm_d3d8_reset_state();
	g_classic_environment_mapper_probe_json.clear();
	...
	if (set_device_result == WW3D_ERROR_OK) {
		ClassicEnvironmentMapperClass *mapper = W3DNEW ClassicEnvironmentMapperClass(kStage);
		...
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"classic_environment_mapper_apply_probe\","
			"\"ok\":%s,"
			...
			static_cast<unsigned long>(D3DTSS_TCI_CAMERASPACENORMAL),
		...
		g_classic_environment_mapper_probe_json = buffer;
	...
	if (g_classic_environment_mapper_probe_json.empty()) {
		char buffer[512];
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"classic_environment_mapper_apply_probe\",\"ok\":false,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d,"
			"\"stage\":%u,\"mapperCreated\":%s,\"applyCalled\":%s}}",
			init_result,
			set_device_result,
	...
	return g_classic_environment_mapper_probe_json.c_str();
}
