...
std::string g_screen_mapper_probe_json;
...
EMSCRIPTEN_KEEPALIVE const char *cnc_port_probe_screen_mapper_apply()
{
	initMemoryManager();
	wasm_d3d8_reset_state();
	g_screen_mapper_probe_json.clear();
	...
	const Matrix4x4 projection(
		1.25f, 0.10f, 0.00f, 0.00f,
		0.05f, 1.50f, 0.00f, 0.00f,
		0.00f, 0.00f, 0.80f, 1.00f,
		0.20f, 0.30f, 0.40f, 1.00f);
	const Vector2 offset_per_sec(0.0f, 0.0f);
	const Vector2 start_offset(0.25f, 0.50f);
	const Vector2 scale(0.50f, 0.75f);
	...
	if (set_device_result == WW3D_ERROR_OK) {
		...
		ScreenMapperClass *mapper =
			W3DNEW ScreenMapperClass(offset_per_sec, start_offset, false, scale, kStage);
		mapper_created = mapper != nullptr;
		mapper_id_ok = mapper != nullptr &&
			mapper->Mapper_ID() == TextureMapperClass::MAPPER_ID_SCREEN;
		...
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"screen_mapper_apply_probe\","
			"\"ok\":%s,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d,"
			"\"stage\":%u,\"mapperCreated\":%s,\"mapperIdOk\":%s,\"applyCalled\":%s},"
		...
		g_screen_mapper_probe_json = buffer;
	...
	if (g_screen_mapper_probe_json.empty()) {
		char buffer[512];
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"screen_mapper_apply_probe\",\"ok\":false,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d,"
			"\"stage\":%u,\"mapperCreated\":%s,\"mapperIdOk\":%s,\"applyCalled\":%s}}",
			init_result,
			set_device_result,
	...
	return g_screen_mapper_probe_json.c_str();
}
