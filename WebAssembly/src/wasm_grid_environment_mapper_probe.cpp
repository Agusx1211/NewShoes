...
std::string g_grid_environment_mapper_probe_json;
...
TextureMapperClass *make_grid_mapper(
	bool reflection,
	unsigned stage,
	unsigned grid_width_log2,
	unsigned last_frame,
	unsigned offset)
{
	...
	if (reflection) {
		return W3DNEW GridEnvironmentMapperClass(
			kStaticFPS, grid_width_log2, last_frame, offset, stage);
	}
	return W3DNEW GridClassicEnvironmentMapperClass(
		kStaticFPS, grid_width_log2, last_frame, offset, stage);
}

GridMapperCaseResult run_grid_mapper_case(
	const char *name,
	bool reflection,
	unsigned stage,
	unsigned grid_width_log2,
	...
	unsigned offset)
{
	...
	result.expected_mapper_id = reflection ?
		TextureMapperClass::MAPPER_ID_GRID_ENVIRONMENT :
		TextureMapperClass::MAPPER_ID_GRID_CLASSIC_ENVIRONMENT;
	...
	TextureMapperClass *mapper =
		make_grid_mapper(reflection, stage, grid_width_log2, last_frame, offset);
...
bool case_ok(const GridMapperCaseResult &result)
{
...
void append_case_json(char *buffer, std::size_t buffer_size, const GridMapperCaseResult &result)
{
...
EMSCRIPTEN_KEEPALIVE const char *cnc_port_probe_grid_environment_mapper_apply()
{
	initMemoryManager();
	wasm_d3d8_reset_state();
	g_grid_environment_mapper_probe_json.clear();
	...
	if (set_device_result == WW3D_ERROR_OK) {
		...
		GridMapperCaseResult classic = run_grid_mapper_case(
			"classic", false, kStage, kGridWidthLog2, kLastFrame, 5);
		GridMapperCaseResult reflection = run_grid_mapper_case(
			"reflection", true, kStage, kGridWidthLog2, kLastFrame, 10);
		...
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"grid_environment_mapper_apply_probe\","
			"\"ok\":%s,"
			...
			classic_json,
			reflection_json);
		g_grid_environment_mapper_probe_json = buffer;
	}
	...
	if (g_grid_environment_mapper_probe_json.empty()) {
		char buffer[256];
		std::snprintf(buffer, sizeof(buffer),
			"{\"source\":\"grid_environment_mapper_apply_probe\",\"ok\":false,"
			"\"results\":{\"init\":%d,\"setRenderDevice\":%d}}",
			init_result,
			set_device_result);
		g_grid_environment_mapper_probe_json = buffer;
	}
	return g_grid_environment_mapper_probe_json.c_str();
}
