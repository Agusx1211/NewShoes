...
const screenshotPath = resolve(screenshotDir, "harness-smoke-bink-videoplayer-sidecar-copy.png");
...
try {
  ...
  const moduleUrl = new URL("dist/bink-videoplayer-browser-runtime-smoke.js", server.url).href;
  const runtimeResult = await page.evaluate(async ({ moduleUrl, gcBytes, vsBytes, manifestText, sidecarPayloads }) => {
    ...
    const createModule =
      moduleExports.default ?? moduleExports.createBinkVideoPlayerBrowserRuntimeSmokeModule;
    ...
    const status = module.ccall(
      "run_bink_videoplayer_sidecar_copy_bridge_smoke",
      "number",
      [],
      [],
    );
  ...
  if (!runtimeResult.ok) {
    throw new Error(`BinkVideoPlayer browser runtime smoke failed: ${JSON.stringify(runtimeResult)}`);
  }
  ...
  if (openEvents.length !== 2 || closeEvents.length !== 2) {
    throw new Error(
      `Expected two BinkVideoPlayer-owned open/close events, got ` +
  ...
  for (const expected of [
    ...
    if (!match) {
      throw new Error(`Missing BinkVideoPlayer sidecar open event: ${JSON.stringify(expected)} in ${JSON.stringify(openEvents)}`);
    }
  }

  for (const type of ["doFrame", "copyPending", "copyComplete", "close"]) {
    const count = binkEvents.filter((event) => event.type === type).length;
    if (count !== 2) {
      throw new Error(`Expected two BinkVideoPlayer browser ${type} events, got ${count}: ${JSON.stringify(binkEvents)}`);
    }
  }
  const nextFrameCount = binkEvents.filter((event) => event.type === "nextFrame").length;
  if (nextFrameCount !== 2) {
    throw new Error(`Expected two BinkVideoPlayer browser nextFrame events, got ${nextFrameCount}: ${JSON.stringify(binkEvents)}`);
  }
  const gotoFrameCount = binkEvents.filter((event) => event.type === "gotoFrame").length;
  if (gotoFrameCount !== 4) {
    throw new Error(`Expected four BinkVideoPlayer browser gotoFrame events, got ${gotoFrameCount}: ${JSON.stringify(binkEvents)}`);
  }

  const copyEvents = runtimeResult.copyEvents ?? [];
  if (copyEvents.length !== 2) {
    throw new Error(`Expected two BinkVideoPlayer sidecar copy hook events, got ${copyEvents.length}: ${JSON.stringify(copyEvents)}`);
  }
  for (const event of copyEvents) {
    if (event.bytesWritten <= 0 || event.checksum === 0) {
      throw new Error(`BinkVideoPlayer sidecar copy hook did not write decoded pixels: ${JSON.stringify(event)}`);
    }
  }
  ...
  console.log(JSON.stringify({
    ok: true,
    source: "WebAssembly/harness/bink_videoplayer_sidecar_browser_smoke.mjs",
    runtimeResult,
    manifestPath,
    screenshotPath,
    browserEvents,
  ...
  throw new Error(
    `BinkVideoPlayer sidecar browser smoke failed: ${error?.message ?? String(error)}; ` +
