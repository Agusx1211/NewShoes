...
const rulerScreenshot = resolve(
  screenshotDir,
  "harness-smoke-ww3d-display-main-menu-ruler-canvas.png",
);
...
const runtimeArchivePath = "/assets/runtime-main-menu-ruler-mapped-image";
...
try {
  ...
  const bootResult = await page.evaluate(() => window.CnCPort.rpc("boot", {
    source: "MainMenuRuler mapped-image W3DDisplay drawImage smoke",
  }));
  if (!bootResult.ok || bootResult.state.wasm !== "loaded") {
    throw new Error(`cnc-port boot failed before MainMenuRuler archive mount: ${JSON.stringify(bootResult)}`);
  }
  ...
  const mappedImageEntry = rangeIniArchive?.entries?.find((entry) =>
    entry.path.toLowerCase() === rulerMappedImageEntry.toLowerCase());
  const textureEntry = rangeTextureArchive?.entries?.find((entry) =>
  ...
  if (!archiveMountResult.ok
      ...
      || archiveMountResult.archiveSet?.sourceTotalBytes !== iniArchiveStat.size + textureArchiveStat.size
      ...
      || mappedImageEntry?.sourceOffset !== 4377432
      || mappedImageEntry?.bytes !== 4304
      || mappedImageEntry?.reader !== "browser fetch Range"
      || textureEntry?.sourceOffset !== 152340144
      || textureEntry?.bytes !== 4194348
      || textureEntry?.reader !== "browser fetch Range") {
    throw new Error(`range-backed MainMenuRuler archive subset mount failed: ${JSON.stringify(archiveMountResult)}`);
  }

  let renderResult;
  try {
    renderResult = await withTimeout(
      page.evaluate((payload) => window.CnCPort.rpc("ww3dDisplayMainMenuRuler", payload), {
        iniArchivePath: iniArchiveMemfsPath,
        textureArchivePath: textureArchiveMemfsPath,
      }),
      30000,
      "MainMenuRuler W3DDisplay drawImage",
    );
  } catch (error) {
    throw new Error(`MainMenuRuler W3DDisplay drawImage crashed: ${error?.message ?? String(error)}; browser events: ${JSON.stringify(browserEvents)}`);
  }
  ...
  if (!renderResult.ok
      || renderResult.command !== "ww3dDisplayMainMenuRuler"
      || renderResult.probe?.source !== "ww3d_display_main_menu_ruler_probe"
      ...
      || String(renderResult.probe?.texture?.name ?? "").toLowerCase() !== "mainmenuruleruserinterface.tga"
      || renderResult.probe?.texture?.archiveEntry !== rulerTextureEntry
      || renderResult.probe?.texture?.width !== 1024
      ...
      || renderResult.probe?.image?.name !== "MainMenuRuler"
      || renderResult.probe?.image?.filename !== "MainMenuRuleruserinterface.tga"
      || renderResult.probe?.image?.rawTexture !== false
      || renderResult.probe?.image?.status !== 0
      || renderResult.probe?.image?.rotated !== false
      || renderResult.probe?.image?.textureWidth !== 1024
      || renderResult.probe?.image?.textureHeight !== 1024
      || !nearlyEqual(renderResult.probe?.image?.uvLoX ?? 0, 0)
      || !nearlyEqual(renderResult.probe?.image?.uvLoY ?? 0, 0)
      || !nearlyEqual(renderResult.probe?.image?.uvHiX ?? 0, 800 / 1024)
      || !nearlyEqual(renderResult.probe?.image?.uvHiY ?? 0, 600 / 1024)
      || renderResult.probe?.image?.width !== 800
      || renderResult.probe?.image?.height !== 600
      || renderResult.probe?.draw?.screenRect?.left !== 0
      || renderResult.probe?.draw?.screenRect?.top !== 0
      || renderResult.probe?.draw?.screenRect?.right !== 800
      || renderResult.probe?.draw?.screenRect?.bottom !== 600
      ...
      || renderResult.coloredRulerPixelCount < 4
      || Object.values(renderResult.rulerPixels ?? {}).filter((pixel) => pixelHasColor(pixel, 8)).length < 4
    ...
    throw new Error(`MainMenuRuler W3DDisplay drawImage failed: ${JSON.stringify(renderResult)}`);
  }

  await page.locator("#viewport").screenshot({ path: rulerScreenshot });

  console.log(JSON.stringify({
    ok: true,
    path: "browser-ww3d-display-main-menu-ruler",
    url: harnessUrl,
    archiveSet: archiveMountResult.archiveSet,
    screenshot: rulerScreenshot,
    probe: renderResult.probe,
    browserProbe: renderResult.browserProbe,
    rulerPixels: renderResult.rulerPixels,
    coloredRulerPixelCount: renderResult.coloredRulerPixelCount,
    textureDelta: renderResult.textureDelta,
    originalPaths: [
      "HandCreatedMappedImages.INI -> ImageCollection::findImageByName(MainMenuRuler)",
      "W3DDisplay::drawImage -> TextureClass::Init(MainMenuRuleruserinterface.tga)",
      "TexturesZH.big Art\\Textures\\mainmenuruleruserinterface.tga -> browser WebGL2 texture",
    ],
    reader: "browser Range subset BIG loaded by runtime-owned Win32BIGFileSystem",
    renderer: "MainMenuRuler mapped image through W3DDisplay::drawImage, TextureClass, and browser D3D8/WebGL2 bridge",
  }, null, 2));
} finally {
  await browser?.close();
  await server.close();
