...
const runtimeArchivePath = "/assets/runtime-window-layout-repaint";
const windowArchiveMemfsPath = `${runtimeArchivePath}/WindowZH.big`;
const layoutEntry = "Window\\Menus\\Defeat.wnd";
...
try {
  ...
  const bootResult = await page.evaluate(() => window.CnCPort.rpc("boot", {
    source: "W3D WindowLayout repaint render smoke",
  }));
  if (!bootResult.ok || bootResult.state.wasm !== "loaded") {
    throw new Error(`cnc-port boot failed before W3D WindowLayout repaint: ${JSON.stringify(bootResult)}`);
  }
  ...
  try {
    repaintResult = await withTimeout(
      page.evaluate((payload) => window.CnCPort.rpc("ww3dWindowLayoutRepaint", payload), {
        windowArchivePath: windowArchiveMemfsPath,
      }),
      45000,
      "W3D WindowLayout repaint",
    );
  } catch (error) {
    throw new Error(`W3D WindowLayout repaint crashed: ${error?.message ?? String(error)}; browser events: ${JSON.stringify(browserEvents)}`);
  }

  if (!repaintResult.ok
      || repaintResult.command !== "ww3dWindowLayoutRepaint"
      || repaintResult.probe?.source !== "ww3d_window_layout_repaint_probe"
      ...
      || repaintResult.probe?.layout?.path !== "Menus/Defeat.wnd"
      || repaintResult.probe?.layout?.root?.systemFunc !== "GameWinDefaultSystem"
      || repaintResult.probe?.layout?.parent?.systemFunc !== "GameWinDefaultSystem"
      ...
      || repaintResult.layoutPixels?.parentInterior?.[2] < 140
      || repaintResult.layoutPixels?.outside?.some((component, index) => index < 3 && component > 8)) {
    throw new Error(`W3D WindowLayout repaint render failed: ${JSON.stringify({
      ok: repaintResult.ok,
  ...
  if (browserFailures.length > 0) {
    throw new Error(`browser failures during W3D WindowLayout repaint: ${JSON.stringify(browserFailures)}`);
  }

  console.log(JSON.stringify({
    ok: true,
    path: "browser-ww3d-window-layout-repaint",
    url: harnessUrl,
    ...
    layoutPixels: repaintResult.layoutPixels,
    renderer: "WindowLayout::load from WindowZH.big through GameWindowManager::winRepaint, W3DGameWinDefaultDraw, W3DDisplay, and browser D3D8/WebGL2 bridge",
