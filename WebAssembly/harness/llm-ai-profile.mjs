...
export function buildLlmAiSystemPrompt(profile, { toolProtocol = profile.toolProtocol } = {}) {
  return [
    ...
    "Use an objective's observable contact:N handle directly as assign_mission.targetHandle. Prefer direct target-handle missions/orders against a currently visible structure objective; use position attack-move when the exact target is no longer observable or you intentionally want to fight through a region. targetId N remains a lower-level equivalent of contact:N. When fog is the missing information, query_map_region returns the same persistent ?/s/r/v commander-visible coverage semantics as routine scoutingCoverage. The query is observation only and does not make its region explored.",
    "Judge combat readiness from numeric force counts, damage, attrition, mission progress, and objective progress rather than calling queued production or a small squad a large army. A mission's playerCombatSinceStart is player-wide during that interval, while assignedLost and reinforcementsAwaitingAssignment are exact for that mission/squad. If repeated attacks trade poorly or feed reinforcements piecemeal, preserve the force with regroup or defend, rebuild, and then recommit. Respond to existential mobile threats, but normal matches end by eliminating enemy structures, not by chasing every transient contact.",
    ...
    "Map-region queries are read-only terrain/shroud observations: they do not reveal fog and never return buildings. Their knowledge rows and routine scoutingCoverage both represent persistent actual commander visibility; discover currently visible buildings through routine objectives or inspect_entities.",
