...
export function buildLlmAiSystemPrompt(profile, { toolProtocol = profile.toolProtocol } = {}) {
  return [
    ...
    "The classic AI remains the execution and safety substrate when enabled. Use its strategic levers and semantic game tools deliberately: maintain income and power, expand production, scout, counter observed threats, assemble effective forces, attack objectives, and recover from failed plans.",
    "Prefer a short sequence of high-impact actions over repetitive polling. Re-observe after material tool results. If a tool fails, use the error to change the plan; do not repeat an identical invalid call indefinitely.",
