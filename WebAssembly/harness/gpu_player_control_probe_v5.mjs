// GPU probe v5: correct field paths for summary frames
...
async function clickButton(page, button, hitProbe, label) {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (f.frame?.clientState?.input?.grabWindow?.id === tgt?.id) break;
  }
...
function checkPlayerControl(pc) {
  // pc = frame.playerControl from summary frame
...
async function main() {
  ...
  // Boot
  const boot = await rpc(page, "boot", { source: "gpu probe v5" });
  ...
  // Wait for campaign game start using full frames (need menu.debug state)
  const bl = Number(mm?.debug?.doGameStartCount ?? 0);
  ...
  for (let i = 0; i < 500; i++) {
    ...
    if (i % 100 === 0 && i > 0) {
      console.error(`[gpu] campaign wait ${i}: doGS=${doGS} cd=${cd} diff=${diff} mode=${mode} pf=${pf} sm=${sm}`);
    }
  }
  if (!campaignStarted) {
    console.error("[gpu] WARNING: campaign start not detected, continuing anyway...");
  }

  // Screenshot at campaign start
  ...
  // Final full frame for rich state export
  fr = await rpc(page, "realEngineFrame", { frames: 1 });

  // Extract state from full frame
