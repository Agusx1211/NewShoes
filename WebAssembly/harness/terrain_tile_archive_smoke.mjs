...
const terrainScreenshot = resolve(
  screenshotDir,
  "harness-smoke-ww3d-terrain-tile-archive-canvas.png",
);

const runtimeArchivePath = "/assets/runtime-terrain-tile";
...
try {
  ...
  const bootResult = await page.evaluate(() => window.CnCPort.rpc("boot", {
    source: "W3D archive terrain tile render smoke",
  }));
  if (!bootResult.ok || bootResult.state.wasm !== "loaded") {
    throw new Error(`cnc-port boot failed before W3D archive terrain tile: ${JSON.stringify(bootResult)}`);
  }
  ...
  const terrainResult = await page.evaluate((payload) =>
    window.CnCPort.rpc("ww3dTerrainTileArchive", payload), {
      terrainArchivePath: terrainArchiveMemfsPath,
    });

  if (!terrainResult.ok
      || terrainResult.command !== "ww3dTerrainTileArchive"
      || terrainResult.probe?.source !== "ww3d_terrain_tile_archive_probe"
    ...
    throw new Error(`W3D archive terrain tile render failed: ${JSON.stringify({
      ok: terrainResult.ok,
  ...
  if (browserFailures.length > 0) {
    throw new Error(`browser failures during W3D archive terrain tile: ${JSON.stringify(browserFailures)}`);
  }

  console.log(JSON.stringify({
    ok: true,
    path: "browser-ww3d-terrain-tile-archive",
    url: harnessUrl,
    ...
    centerPixel: terrainResult.screenshot.centerPixel,
    renderer: "TerrainZH.big TGA -> original WorldHeightMap::readTiles -> W3DTerrainBackground stage-1 texture bind/sample -> browser D3D8/WebGL2",
