// GPU probe v4: clean chunked approach
...
async function main() {
  ...
  // Boot
  const boot = await rpc(page, "boot", { source: "gpu probe v4" });
  ...
  fr = await rpc(page, "realEngineFrame", { frames: 1 }); // get fresh state

  const mm2 = fr.frame?.clientState?.mainMenu;
  await clickButton(page, mm2?.buttonUSA, mm2?.underButtonUSACenter, "USA");
  fr = await rpc(page, "realEngineFrame", { frames: 1 });

  const mm3 = fr.frame?.clientState?.mainMenu;
  await clickButton(page, mm3?.buttonEasy, mm3?.underButtonEasyCenter, "Easy");
  ...
  console.error(`[gpu] total ${totalFrames}f in ${wallMs}ms (${(totalFrames*1000/wallMs).toFixed(1)}fps)`);

  // Final full frame for rich state
  fr = await rpc(page, "realEngineFrame", { frames: 1 });
