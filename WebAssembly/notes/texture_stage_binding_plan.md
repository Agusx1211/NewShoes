...
- The D3D base texture pointer is what the browser upload plumbing already
  produces; the shim must own the **bind** step, translating
  `Set_DX8_Texture(stage, tex)` into "bind this uploaded WebGL texture to
  `gl.TEXTURE0 + stage`".
...
- [ ] **S1 — `Set_DX8_Texture` bind route (bind-only).** In the shim, implement
      `Set_DX8_Texture(stage, d3dBaseTex)`: resolve `d3dBaseTex` → WebGL texture
      (the upload plumbing's handle) → `activeTexture(TEXTURE0+stage)` +
      `bindTexture(target, handle)`. Mirror the `Textures[MAX_TEXTURE_STAGES]`
      redundant guard. Null → `bindTexture(target, null)`. Verify with a
      single-textured 2D blit screenshot.
