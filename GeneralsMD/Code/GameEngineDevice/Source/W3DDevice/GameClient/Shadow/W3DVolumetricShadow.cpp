...
#else
#define CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP(name) do { } while (0)
#define CNC_PORT_SHADOW_PROFILE_SUBMIT_ENABLED() false
#define CNC_PORT_BEGIN_SHADOW_SUBMIT_PROFILE_SCOPE(enabled) do { } while (0)
#define CNC_PORT_END_SHADOW_SUBMIT_PROFILE_SCOPE(enabled) do { } while (0)
#endif
...
#ifdef __EMSCRIPTEN__
const Matrix3D *W3DVolumetricShadow::dynamicShadowMeshTransform(Int meshIndex) const
{
	if (!m_geometry || !m_robj || meshIndex < 0 || meshIndex >= m_geometry->getMeshCount())
		return NULL;
	...
	return mesh ? &mesh->Get_Transform() : NULL;
}
#endif
...
{
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.entry");
	Geometry *geometry;
	...
	if (!m_pDev)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.noDevice.return");
		return;
	}
	...
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.empty.return");
		return;
	}

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.transform.before");
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.transform.after");
	
	W3DBufferManager::W3DVertexBufferSlot *vbSlot=m_shadowVolumeVB[lightIndex][ meshIndex ];
	if (!vbSlot)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.noVB.return");
		return;
	}
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.vbBind.before");
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.vbBind.after");
	...
	if (!ibSlot)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.noIB.return");
		return;
	}

	DEBUG_ASSERTCRASH(ibSlot->m_size >= numIndex,("Overflowing Shadow Index Buffer Slot"));

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.ibBind.before");
	m_pDev->SetIndices(ibSlot->m_IB->m_DX8IndexBuffer->Get_DX8_Index_Buffer(),vbSlot->m_start);
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderMeshVolume.ibBind.after");
...
void W3DVolumetricShadow::RenderDynamicMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D *meshXform, Bool reuseUploadedRange)
{
	...
	if (!m_pDev)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.noDevice.return");
		return;
	}
	...
	if( numVerts == 0 || numPolys == 0 )
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.empty.return");
		return;
	}
	...
	{	//flush the buffer by drawing the contents and re-locking again
		...
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vbLock.failed");
			return;
		}
		nShadowVertsInBuf=0;
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vbLock.after");
#ifdef SV_DEBUG
	srand(0x1345465);
#endif
	if(pvVertices)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vertexWrite.before");
#ifdef SV_DEBUG
...
#endif
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vertexWrite.after");
	}

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vbUnlock.before");
	shadowVertexBufferD3D->Unlock();
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.vbUnlock.after");

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.ibLock.before");
	...
	{	//flush the buffer by drawing the contents and re-locking again
		...
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.ibLock.failed");
			return;
		}
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.ibLock.after");


	try {
	if(pvIndices)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.indexWrite.before");
		memcpy(pvIndices,geometry->GetPolygonIndex(0,(short *)pvIndices),numPolys*3*sizeof(short));
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.indexWrite.after");
	}
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.bind.after");
...
#ifdef __EMSCRIPTEN__
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchMember = FALSE;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchHead = FALSE;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchVertexStart = 0;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchVertexCount = 0;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchIndexStart = 0;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchPolyCount = 0;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchVertexGeneration = 0;
			m_shadowVolumeRenderTask[i][j].m_dynamicWorldBatchIndexGeneration = 0;
#endif
...
{
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.entry");
	LPDIRECT3DDEVICE8 m_pDev=DX8Wrapper::_Get_D3D_Device8();

	if (!m_pDev)
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.noDevice.return");
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.state.before");
...
//  m_pDev->SetRenderState( D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA );
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.state.after");

	if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
	{
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.drawUP.before");
		m_pDev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, v, sizeof(_TRANSLITVERTEX));
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.drawUP.after");
	}

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.cleanup.before");
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.cleanup.after");
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderStencilShadows.complete");

}  // end renderStencilShadows

#ifdef __EMSCRIPTEN__
const Matrix3D *W3DVolumetricShadowManager::dynamicShadowTaskMeshTransform(W3DVolumetricShadowRenderTask *task) const
{
	if (!task || !task->m_parentShadow)
		return NULL;

	return task->m_parentShadow->dynamicShadowMeshTransform(task->m_meshIndex);
}

void W3DVolumetricShadowManager::renderDynamicShadowWorldBatch(Int vertexStart, Int vertexCount, Int indexStart, Int polyCount)
{
	LPDIRECT3DDEVICE8 m_pDev = DX8Wrapper::_Get_D3D_Device8();

	if (!m_pDev || !shadowVertexBufferD3D || !shadowIndexBufferD3D ||
		vertexCount <= 0 || polyCount <= 0)
		return;

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.worldBatchReplay.before");
	...
	m_pDev->SetIndices(shadowIndexBufferD3D,vertexStart);
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.worldBatchReplay.after");

	if (DX8Wrapper::_Is_Triangle_Draw_Enabled())
	{
		Debug_Statistics::Record_DX8_Polys_And_Vertices(polyCount,vertexCount,ShaderClass::_PresetOpaqueShader);
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.draw.before");
		const bool profile_submit = CNC_PORT_SHADOW_PROFILE_SUBMIT_ENABLED();
		CNC_PORT_BEGIN_SHADOW_SUBMIT_PROFILE_SCOPE(profile_submit);
		m_pDev->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,vertexCount,indexStart,polyCount);
		CNC_PORT_END_SHADOW_SUBMIT_PROFILE_SCOPE(profile_submit);
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.draw.after");
	}
}
...
Int W3DVolumetricShadowManager::renderDynamicShadowTasks(void)
{
	...
	while (task)
	{
		Geometry *firstGeometry = task->m_parentShadow->m_shadowVolume[task->m_lightIndex][task->m_meshIndex];
		Int firstNumVerts = firstGeometry ? firstGeometry->GetNumActiveVertex() : 0;
		Int firstNumPolys = firstGeometry ? firstGeometry->GetNumActivePolygon() : 0;
		Int firstNumIndex = firstNumPolys * 3;
		if (firstNumVerts == 0 || firstNumPolys == 0)
		{
			task->m_dynamicBufferRangeValid = FALSE;
			task = (W3DVolumetricShadowRenderTask *)task->m_nextTask;
			continue;
		}

		if (firstNumVerts > SHADOW_VERTEX_SIZE || firstNumIndex > SHADOW_INDEX_SIZE)
		{
...
#ifdef __EMSCRIPTEN__
		if (!dynamicShadowTaskMeshTransform(task))
		{
			task->m_dynamicBufferRangeValid = FALSE;
			task->m_parentShadow->RenderVolume(task->m_meshIndex,task->m_lightIndex);
			renderedCount++;
		...
		const Bool discard = (nShadowVertsInBuf > (SHADOW_VERTEX_SIZE - firstNumVerts)) ||
			(nShadowIndicesInBuf > (SHADOW_INDEX_SIZE - firstNumIndex));
...
#endif
		W3DVolumetricShadowRenderTask *batchEnd = task;

		while (batchEnd)
		{
			Geometry *geometry = batchEnd->m_parentShadow->m_shadowVolume[batchEnd->m_lightIndex][batchEnd->m_meshIndex];
			...
			if (numVerts == 0 || numPolys == 0 ||
				numVerts > SHADOW_VERTEX_SIZE || numIndex > SHADOW_INDEX_SIZE)
			{
				break;
			}
#ifdef __EMSCRIPTEN__
			if (!dynamicShadowTaskMeshTransform(batchEnd))
			{
				break;
			}
#endif

			if (batchVertexCount + numVerts > SHADOW_VERTEX_SIZE - batchVertexStart ||
				batchIndexCount + numIndex > SHADOW_INDEX_SIZE - batchIndexStart)
			{
				break;
			}

			batchVertexCount += numVerts;
			batchIndexCount += numIndex;
#ifdef __EMSCRIPTEN__
			batchTaskCount++;
#endif
			batchEnd = (W3DVolumetricShadowRenderTask *)batchEnd->m_nextTask;
		}

		if (batchVertexCount <= 0 || batchIndexCount <= 0)
		{
			task->m_parentShadow->RenderVolume(task->m_meshIndex,task->m_lightIndex);
			renderedCount++;
			task = (W3DVolumetricShadowRenderTask *)task->m_nextTask;
		...
		if (discard)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbLockDiscard.before");
		}
		else
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbLockNoOverwrite.before");
		}

		if (shadowVertexBufferD3D->Lock(batchVertexStart*sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX),
			batchVertexCount*sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX),(unsigned char **)&pvVertices,lockFlags) != D3D_OK)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbLock.failed");
			task->m_parentShadow->RenderVolume(task->m_meshIndex,task->m_lightIndex);
			renderedCount++;
			task = (W3DVolumetricShadowRenderTask *)task->m_nextTask;
			continue;
		...
		if (discard)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbLockDiscard.after");
		}
		else
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbLockNoOverwrite.after");
		}

		if (discard)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbLockDiscard.before");
		}
		else
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbLockNoOverwrite.before");
		}

		if (shadowIndexBufferD3D->Lock(batchIndexStart*sizeof(short),
			batchIndexCount*sizeof(short),(unsigned char **)&pvIndices,lockFlags) != D3D_OK)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbLock.failed");
			shadowVertexBufferD3D->Unlock();
			task->m_parentShadow->RenderVolume(task->m_meshIndex,task->m_lightIndex);
			renderedCount++;
			task = (W3DVolumetricShadowRenderTask *)task->m_nextTask;
		...
		if (discard)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbLockDiscard.after");
		}
		else
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbLockNoOverwrite.after");
		}
		...
		Int vertexStart = batchVertexStart;
		Int indexStart = batchIndexStart;
		W3DVolumetricShadowRenderTask *writeTask = task;

		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVertexWrite.before");
		while (writeTask != batchEnd)
		{
			Geometry *geometry = writeTask->m_parentShadow->m_shadowVolume[writeTask->m_lightIndex][writeTask->m_meshIndex];
			Int numVerts = geometry->GetNumActiveVertex();
#ifdef __EMSCRIPTEN__
			const Matrix3D *meshXform = dynamicShadowTaskMeshTransform(writeTask);
#ifdef SV_DEBUG
			srand(0x1345465);
#endif
			for (Int i=0; i<numVerts; i++)
			...
			for (Int i=0; i<numVerts; i++)
			{
				(*((Vector3 *)vertexCursor))=*geometry->GetVertex(i);	//cast is valid since both start with xyz
				vertexCursor->diffuse=(rand()%255) | ((rand()%255)<<8) | ((rand()%255)<<16);
				vertexCursor++;
			}
#else
			memcpy(vertexCursor,geometry->GetVertex(0),numVerts*sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX));
			vertexCursor += numVerts;
#endif
#endif
			writeTask = (W3DVolumetricShadowRenderTask *)writeTask->m_nextTask;
		}
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVertexWrite.after");

		try {
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIndexWrite.before");
		writeTask = task;
#ifdef __EMSCRIPTEN__
		Int indexVertexStart = batchVertexStart;
#endif
		while (writeTask != batchEnd)
		{
			Geometry *geometry = writeTask->m_parentShadow->m_shadowVolume[writeTask->m_lightIndex][writeTask->m_meshIndex];
...
#ifdef __EMSCRIPTEN__
			const Int vertexBaseOffset = indexVertexStart - batchVertexStart;
			short indexList[3];
			...
			indexVertexStart += geometry->GetNumActiveVertex();
#else
			Int numIndex = numPolys * 3;
			memcpy(indexCursor,geometry->GetPolygonIndex(0,(short *)indexCursor),numIndex*sizeof(short));
			indexCursor += numIndex;
#endif
			writeTask = (W3DVolumetricShadowRenderTask *)writeTask->m_nextTask;
		}
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIndexWrite.after");
		IndexBufferExceptionFunc();
		} catch(...) {
			IndexBufferExceptionFunc();
		}
		...
		while (writeTask != batchEnd)
		{
			...
			writeTask->m_dynamicVertexStart = vertexStart;
			writeTask->m_dynamicVertexCount = numVerts;
			writeTask->m_dynamicIndexStart = indexStart;
			writeTask->m_dynamicIndexCount = numIndex;
			writeTask->m_dynamicPolyCount = numPolys;
			writeTask->m_dynamicVertexGeneration = s_dynamicShadowVertexBufferGeneration;
			writeTask->m_dynamicIndexGeneration = s_dynamicShadowIndexBufferGeneration;
#ifdef __EMSCRIPTEN__
			writeTask->m_dynamicBufferRangeValid = FALSE;
			writeTask->m_dynamicWorldBatchMember = TRUE;
			writeTask->m_dynamicWorldBatchHead = FALSE;
#else
			writeTask->m_dynamicBufferRangeValid = TRUE;
#endif
...
#ifdef __EMSCRIPTEN__
		task->m_dynamicWorldBatchHead = TRUE;
		task->m_dynamicWorldBatchVertexStart = batchVertexStart;
		task->m_dynamicWorldBatchVertexCount = batchVertexCount;
		task->m_dynamicWorldBatchIndexStart = batchIndexStart;
		task->m_dynamicWorldBatchPolyCount = batchIndexCount / 3;
		task->m_dynamicWorldBatchVertexGeneration = s_dynamicShadowVertexBufferGeneration;
		task->m_dynamicWorldBatchIndexGeneration = s_dynamicShadowIndexBufferGeneration;
#endif

		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbUnlock.before");
		shadowVertexBufferD3D->Unlock();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchVbUnlock.after");

		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbUnlock.before");
		shadowIndexBufferD3D->Unlock();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderDynamicVolume.batchIbUnlock.after");

		nShadowVertsInBuf = batchVertexStart + batchVertexCount;
		nShadowStartBatchVertex = nShadowVertsInBuf;
		nShadowIndicesInBuf = batchIndexStart + batchIndexCount;
		nShadowStartBatchIndex = nShadowIndicesInBuf;

#ifdef __EMSCRIPTEN__
		renderDynamicShadowWorldBatch(batchVertexStart,batchVertexCount,batchIndexStart,batchIndexCount/3);
		renderedCount += batchTaskCount;
#else
		writeTask = task;
		while (writeTask != batchEnd)
		{
			writeTask->m_parentShadow->RenderVolume(writeTask->m_meshIndex,writeTask->m_lightIndex, TRUE);
			renderedCount++;
			writeTask = (W3DVolumetricShadowRenderTask *)writeTask->m_nextTask;
		}
#endif
...
{
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.entry");
	...
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.visibleBox.before");
 	TheTerrainRenderObject->getMaximumVisibleBox(*shadowCameraFrustum, &bbox, TRUE);
	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.visibleBox.after");
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
		...
		if (!m_pDev)
		{
			CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.noDevice.return");
			return;	//need device to render anything.
		}
		...
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.setup.before");
	  ...
	  m_pDev->SetRenderState( D3DRS_ZENABLE,          TRUE );
		...
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.setup.after");
		...
		// Step through each shadow to collect dynamic tasks, then upload them in
		// capacity-bounded chunks before drawing through the normal replay path.
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.dynamicIncrement.before");
		...
		numRenderedShadows += renderDynamicShadowTasks();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.dynamicIncrement.after");
		...
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.dynamicDecrement.before");
...
#ifdef __EMSCRIPTEN__
			if (shadowDynamicTask->m_dynamicWorldBatchHead)
			{
				if (shadowDynamicTask->m_dynamicWorldBatchVertexGeneration == s_dynamicShadowVertexBufferGeneration &&
					shadowDynamicTask->m_dynamicWorldBatchIndexGeneration == s_dynamicShadowIndexBufferGeneration)
				{
					renderDynamicShadowWorldBatch(
						shadowDynamicTask->m_dynamicWorldBatchVertexStart,
						shadowDynamicTask->m_dynamicWorldBatchVertexCount,
						shadowDynamicTask->m_dynamicWorldBatchIndexStart,
						shadowDynamicTask->m_dynamicWorldBatchPolyCount);
				...
				while (shadowDynamicTask &&
					shadowDynamicTask->m_dynamicWorldBatchMember &&
					!shadowDynamicTask->m_dynamicWorldBatchHead);
				continue;
			}
			else if (!shadowDynamicTask->m_dynamicWorldBatchMember)
			{
				shadowDynamicTask->m_parentShadow->RenderVolume(shadowDynamicTask->m_meshIndex,shadowDynamicTask->m_lightIndex, TRUE);
			}
#else
		...
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.resetTasks.before");
		for (nextVb=TheW3DBufferManager->getNextVertexBuffer(NULL,W3DBufferManager::VBM_FVF_XYZ);nextVb != NULL; nextVb=TheW3DBufferManager->getNextVertexBuffer(nextVb,W3DBufferManager::VBM_FVF_XYZ))
		{
			nextVb->m_renderTaskList=NULL;
		}
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.resetTasks.after");

		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.cleanup.before");
		m_pDev->SetRenderState(D3DRS_CULLMODE,D3DCULL_CW);
		...
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.cleanup.after");
...
//		if (numRenderedShadows)
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.composite.before");
			renderStencilShadows();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.composite.after");

		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.postState.before");
		m_pDev->SetRenderState(D3DRS_SHADEMODE, D3DSHADE_GOURAUD);
		m_pDev->SetRenderState(D3DRS_ALPHABLENDENABLE , FALSE);
		m_pDev->SetRenderState(D3DRS_LIGHTING, FALSE);

		DX8Wrapper::Invalidate_Cached_Render_States();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.postState.after");
	}
	else
	if (forceStencilFill)
	{	//no shadows to render, but still need to fill stencil buffer
		...
		//Set W3D to some known state
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.forceFill.before");
		VertexMaterialClass *vmat=VertexMaterialClass::Get_Preset(VertexMaterialClass::PRELIT_DIFFUSE);
		DX8Wrapper::Set_Material(vmat);
		REF_PTR_RELEASE(vmat);
		DX8Wrapper::Set_Shader(ShaderClass::_PresetOpaqueShader);
		...
		DX8Wrapper::Invalidate_Cached_Render_States();
		CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.forceFill.after");
	}

	CNC_PORT_NOTE_VOLUMETRIC_SHADOW_STEP("W3DVolumetricShadow.renderShadows.complete");
}  // end RenderShadows
