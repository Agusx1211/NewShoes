...
{
	typedef typename PrimitiveAnimationChannelClass<T>::KeyClass BaseKeyClass;
	...
	if (time < this->m_Data[key_count - 1].Get_Time ()) {
		...
		BaseKeyClass *key1 = &this->m_Data[this->m_LastIndex];
		BaseKeyClass *key2 = &this->m_Data[key_count - 1];
